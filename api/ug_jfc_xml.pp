...
{DEFINE DIAGMSG}
{$IFDEF DIAGMSG}
{$INFO DIAGNOSTIC MESSAGES ENABLE}
{$ENDIF}
...
var
  ...
  bTimeToGo: boolean;
begin
  NTK:=JFC_TOKENIZER.Create;//(16384,1);
  bInNode:=false;
  bTimeToGo:=false;
  {$IFDEF DIAGMSG}
  writeln('----BEGIN----');
  writeln('JFC_XML.bParseNode(p_TK: JFC_TOKENIZER; p_i4Line: ',p_i4Line, ' bOk: ', sYesNo(bOk),', p_XML: JFC_XML; p_i4Nest: ',p_i4Nest,'): boolean;');
  writeln('----BEGIN----');
  writeln;

  //p_TK.DumpToTextFile('debug_tokenizer.txt');
  //riteln('Dumped p_TK in xml ParseNode. See debug_tokenizer.txt');
  ...
  repeat
    bDone:=false;
    {$IFDEF DIAGMSG}
    writeln('TOKEN >',p_TK.Item_saToken,'<');
    writeln('bInNode: ',sYesNo(bInNode));
    writeln('Item_iQuoteID: ',p_TK.Item_iQuoteID);
    {$ENDIF}
    ...
    begin // We have a node
      ...
      begin
        ...
        NTK.sWhitespace:=' ';
        NTK.sQuotes:='"';
        NTK.bKeepDebugInfo:=true;
        {$IFDEF DIAGMSG}writeln('parsenode - NTK.Tokenize: ' +p_TK.Item_saToken);{$ENDIF}
        NTK.Tokenize(p_TK.Item_saToken);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              // some browsers hate odd characters not in cdata tag.
            end;
          end;
              ...
              repeat
                ...
                begin
                  ...
                  begin
                    ...
                    {$IFDEF DIAGMSG}writeln('Node: '+sNode+' Attribute: '+NTK.Item_saToken);{$ENDIF}
                    if NTK.MoveNext and (NTK.Item_iQuoteID=0) and (NTK.Item_saToken='=') and
                       NTK.MoveNext and (NTK.Item_iQuoteID=-1) then
                    begin
                      {$IFDEF DIAGMSG}writeln('Node: '+sNode+' Attribute Value: '+NTK.Item_saToken);{$ENDIF}
                      JFC_XMLITEM(p_XML.lpItem).AttrXDL.Item_saValue:=saHTMLUnScrub(NTK.Item_saToken);
                    end
                    else
                    begin
...
var 
  ...
  iBookMarkNth: longint;
begin
  {$IFDEF DIAGMSG}writeln('bParseXML - Begin');{$ENDIF}
  iBookMarkNth:=N;
  bOk:=true;
  i4Line:=1;
  saLastError:='';
  TK:=JFC_TOKENIZER.Create;//(16384,1);
  TK.sSeparators:=csCR+csLF+#9;
  //TK.saWhitespace:=' '+#9;
  TK.AppendQuotePair('<![CDATA[',']]>');
  TK.AppendQuotePair('<!--','-->');
  TK.AppendQuotePair('<','>');
  TK.bKeepDebugInfo:=false;
  {$IFDEF DIAGMSG}writeln('bParseXML - Tokenizing...');{$ENDIF}
  TK.Tokenize(p_saXML);
  {$IFDEF DIAGMSG}
  //riteln('back from tokenizing. Making diagnostic dropfile: tk_jfc_xml_test.txt');
  //TK.DumpToTextFile('tk_jfc_xml_test.txt');
  {$ENDIF}
  if TK.MoveFirst then
  begin
    {$IFDEF DIAGMSG}writeln('Diving Deeper');{$ENDIF}
    bOk:=bParseNode(tk,i4Line, bOk, self, 1);
    {$IFDEF DIAGMSG}writeln('Coming Out');{$ENDIF}
  end
  else
  begin
    bOk:=false;
    saLastError:='201004191552 - JFC_XML Parsing Error - Unable to tokenize. Passing empty string causes this also. Data in:'+p_saXML;
  end;
  TK.Destroy;
  FoundNth(iBookMarkNth);
  result:=bOk;  
  {$IFDEF DIAGMSG}writeln('bParseXML - End');{$ENDIF}
end;
//=============================================================================
