...
Type JFC_TOKENIZER = Class(JFC_MATRIX)
//=============================================================================
  {}
...
//  //---------------------------------------------------------------------------
  ...
  Property Tokens: uint read uRows; //< same as listcount in JFC_DL for example.
  Property Token: UInt read N; //< same as "N" - The Sequential Position in JFC_DL for example.
  ...
  ):Boolean;//< Alphabetically sorts the tokens
  {}
...
//IMPLEMENTATION===============================================================
//*****************************************************************************
// !#!JFC_QUOTEXDL ----NOTE: AND ARRAY WAS IMPLEMENTED ON OPTIMIZATION PASS 1
// This is preserved for reference, educational code snippet - whatever :)
// it ran so - its preserved ;)
//*****************************************************************************
//=============================================================================
//*****************************************************************************
...
Begin
  ...
  {$ENDIF}
  result := uFoundItem(1,p_sa,p_bCaseSensitive)>0;
  {$IFDEF DIAGMSG}
...
Begin
  ...
  {$ENDIF}
  result := uFoundNext(1,p_sa,p_bCaseSensitive,N)>0;
  {$IFDEF DIAGMSG}
...
//=============================================================================
Function JFC_TOKENIZER.SortTokens(
  p_bCaseSensitive: Boolean;
  p_bAscending: Boolean
):Boolean;
//=============================================================================
Begin
  {$IFDEF DIAGMSG}
  writeln('BEGIN - JFC_TOKENIZER.SortItem_saName(CasesSensitive: ',sYesNo(p_bCaseSensitive),' Ascending: ',sYesNo(p_bAscending));
  {$ENDIF}
  result:=bSort(1,p_bAscending,p_bCaseSensitive);
  {$IFDEF DIAGMSG}
  writeln('END   - JFC_TOKENIZER.SortItem_saName Success: ',sYesNo(result));
  {$ENDIF}
End;
//=============================================================================
...
Begin
  ...
  Begin
    ...
    try
      ...
      Writeln(f,'<');
      Writeln(f,saDashedLine);
      Writeln(f,'Tokens:',uRows);
      Writeln(f,saDashedLine);
      If ListCount>0 Then
      Begin
        MoveFirst;
        repeat
          Writeln(f,'Token ',N,' >',saHumanReadable(ITem_saToken),'< iQuoteID:',Item_iQuoteID );
        Until not MoveNext;
        Writeln(f,saDashedLine);
      End;
      Writeln(f);
...
Begin
  ...
  uListCountIn:=uRows; // Record How Many Tokens So Far to Calc end result
    ...
    Begin
      ...
      Begin//quotemode
        ...
        Begin
          ...
          Begin // Not End Quote!
            //riteln('NOT END QUOTE:',p_sa[iBP],' Which Quote mode? ',iQuoteMode,' Looks like:',sQuotes[iQuoteMode*-1]);
            Item_saToken:=Item_saToken+p_sa[iBp];
            //iBP+=1;
          End 
          Else 
          Begin
            ...
            Begin // possible escape quote
              //riteln('CHECKING FOR POSSIBLE ESCAPE QUOTE:',sQuotes[iQuoteMode*-1]);
  ...
  If (uRows>0) and (length(Item_saToken)=0) Then
  begin
    DeleteItem;
  end;
  //if(Item_saToken='#@!JEG!@#') then Item_saToken:='';
  
  Result:=uRows-uListCountIn;
  {$IFDEF DIAGMSG}
...
{$ENDIF}
  result:=uFoundItem(3,inttostr(p_i),true)>0;
{$IFDEF DIAGMSG}
