...
//=============================================================================
{ }
// This unit is the CORE Jegas API's Console support. It uses the core runtime
// library units Video, Mouse and Keyboard to provide virtual consoles allowing
// console based GUI (they are still very useful) applications, swimming fish,
// or even aconsole based graphical game. so you basically can have as many
// virtual console as you want, you define the size, even the z order. You can
// even poke holes in them and see thru them!
//
Unit uc_console;
//=============================================================================
...
{$DEFINE LESSFRAG} // Attempts to REUSE Allocated Memory if the new VC Size is
                  // the same size or smaller than it was previously -
...
{DEFINE DROPWIN32} // For Writing Linux Code initially on Win32,
  ...
  // I don't expect the code to run in windows like this, but it lets
  // me do - at least quite a bit of "does it compile" testing in Windows
  // which is the only place I can use my favorite text editor:
  // www.terasoft.ru "Tera Edit" - Awesome Column editing features...
  // anyways back to it... but he dropped it so i had to make my own editor
...
//=============================================================================
  ...
  { }
  // Decides if the scroll region variables are used or if the whole vc (virtual console) will scroll.
...
//=============================================================================
{ }
// This function allows reading and writing to the selected display device.
// Note otspassing the address of functions inside the video driver, to those
// pointner are actual function call addresses in memory. Have no fear,
// if you populate the record with the correct values using:
// GetConsoleDriver(Var Driver: rtConsoleDriver);
// then modify the fields wish. Writing the data back with the function:
// SetConsoleDriver(Const Driver: rtConsoleDriver):Boolean; will store it
// and the changes are put into effect. (perhaps not until a video mode change.
// I always store and then clear thew screen.
Procedure GetConsoleDriver(Var Driver: rtConsoleDriver);
{ }
// popuates a record type name rtConsoleMode. Calling this function will
// have Mode populated with correct data upon its return.
Procedure GetConsoleMode(Var Mode: rtConsoleMode);
{ }
// reports how many console modes this console can support.
Function GetConsoleModeCount: Word;
{ }
// Get Specific information about a particular ConsoleMode supported by the
// console.
Function GetConsoleModeData(Index: Word; Var Data: rtConsoleMode):Boolean;
{ }
// Change the Console Driver.
Function SetConsoleDriver(Const Driver: rtConsoleDriver):Boolean;
{ }
// Updates the console; call after you are donw making drawing calls to see
// them.
...
//=============================================================================
{ }
// Structure used to store keypress information
...
//=============================================================================
  ...
  ConsoleY:LongInt;
  Action  :Word;
  VC      : UINT;  //< Virtual Console the mouse event happened in
  VMX     :LongInt;//< virtual console mouse x position
  VMY     :LongInt;//< virtual console mouse y position
End;
//=============================================================================
...
//=============================================================================
  { }
  rLKey: rtKeyPress;//< record with infor about a single keypress
  rLMEvent: rtMEvent;//< record for a singlemouse event.
  rPollMouseEvent: TMouseEvent; //< Used to store info when polling the mouse
  ConsoleUnitInitialized: Boolean;//< set to true if the console unit is initialized
  giConsoleWidth: LongInt;//< width of console in characters
  giConsoleHeight: LongInt;//< height of console in characters
...
//=*=
{ }
Function KeyHit: Boolean;//< returns true if a key was hit andneeds to be pulled
Procedure GetKeyPress; //< wait for and grab a keypress from the buffer
Procedure TurnOffCNTLC; //< this makes the normal KILL key sequence "CNTL+C" in effective. Careful with this one.
{}
//=============================================================================
...
//=*=
{ }
Function HaveM: Boolean;//< Have Mouse?
Function MButtons: LongInt;//< How many buttons
Procedure HideMCursor;//< hide the mouse cursor
Procedure ShowMCursor;//< show the mouse cursor
Function GetMEvent:rtMEvent;//<return the rtMEvent (Mouse event) structure populated with the next mouse action in thr queue.
Function MEvent: Boolean;//< returns true if a mouse event has occurred
Function GetMX: Word; //< not reliable in windows
Function GetMY: Word; //< not reliable in windows
Function GetMButtons: Word;//< returns bit flags for the mouse buttons - seems like it should scale  to 16 buttons? :)
Procedure SetMXY(x,y:Word);//< set the position of the mouse cursor
//=============================================================================
...
//-----------------------------------------------------------------------------
{ }
// Creates VC with W,H = giConsoleWidth x giConsoleHeight
Function NewVC:UINT;
{ }
//-----------------------------------------------------------------------------
{ }
// create a virtual console to your size specifications
Function NewVC(p_iVWidth, p_iVHeight: LongInt):UINT;
{ }
//-----------------------------------------------------------------------------
{ }
// create a virtual console to your size specifications and position it at x,y
Function NewVC(p_iX, p_iY,p_iVWidth, p_iVHeight: Longint):UINT;
//-----------------------------------------------------------------------------
{ }
// create a virtual console to your size specifications and position it at x,y AND
// fill the virtual console with the specified color properties. Setting up the
// default text and background colors.
Function NewVC(p_iX, p_iY,p_iVWidth, p_iVHeight: Longint;p_u1VFGC, p_u1VBGC: Byte):UINT;
//-----------------------------------------------------------------------------
{ }
Procedure RemoveVC(p_uVCUID: UINT);//< Take the specified Virtual Console unique identified (UID) passed in p_uVCUID and remove it from memory.
Function  GetVCCount: LongInt; //< Returns how many virtual consoles are currently instantiated
Procedure BringVCToTop(p_uVCUID: UINT);//< bring the specified virtual console to the top of the Z-Order, essentially bringing it to the top.
Procedure SetActiveVC(p_uVCUID: UINT);//< If the specified virtual Console exists, than its UID is assigned to the global ActiveVC.
Function  GetActiveVC: UINT; //< Simply returns the current ActiveVC UID.
Procedure VCClear;//<clears the screen of the ActiveVC
Function VCWidth: LongInt;//< returns the width of the ActiveVC
Function VCHeight: LongInt;//< Returns the height of the ActiveVC
Function VCCursorType: Word;//< returns the cursortype of the ActiveVC
Procedure VCSetCursorType(p_wCursorType: Word);//< Allows you to set the cursor type of the active vc.
Procedure VCSetCursorXY(p_iX, p_iY: LongInt);//< use to set the x,y coordinates of the virtual console. The center of the console universe is at 1,1 but your virtual console can be negative X, off to the left off screen, or negative Y up and above the screen, also far right or down as you want ... of course it has to be in the visiable area of your screen to be seen... naturally :)
Function VCGetCursorX: LongInt;//<Return the X position of the ActiveVC
Function VCGetCursorY: LongInt;//<Return the Y position of the ActiveVC

Procedure VCSetFGC(p_u1FGC: Byte);//<Set Foreground color on the ActiveVC
Procedure VCSetBGC(p_u1BGC: Byte);//<Set Background color on the ActiveVC
Procedure VCSetPenXY(p_iX, p_iY:LongInt);//<this sets the Pen position in the active virtual console. This is the same as a cursor position on the screen. The Pen's X and Y location.
Procedure VCSetPenX(p_iX:LongInt); //< Set Pen X position in the ActiveVC
Procedure VCSetPenY(p_iY:LongInt); //< Set Pen Y position in the ActiveVC
Procedure VCPutChar(p_iX, p_iY: LongInt; p_ch:Char);//< place a character at a specific location in the active VC. It will only remember characters actually placed within its current size.
Procedure VCPutBGC(p_iX, p_iY: LongInt; p_u1BGC:Byte);//< place the background color you would like at a specific character cell in the activevc
Procedure VCPutFGC(p_iX, p_iY: LongInt; p_u1FGC:Byte);//< place the foreground color you would like at a specific character cell in the activevc.
{ }
// PolyMorphed VCPutAll
{ }
Procedure VCPutAll(p_iX, p_iY: LongInt; p_wAll:Word);//< fill every character color byte pair (a word) with the value passed in p_wAll in the ActiveVC
Procedure VCPutAll(p_iX, p_iY: LongInt; P_ch: Char; p_byFG, p_byBG: Byte);//< fill every character cell with the specified character, foreground and background colors in the ActiveVC
Function VCGetChar(p_iX, p_iY: LongInt): Char;//< returns character at the specified x,y location in the ActiveVC
Function VCGetBGC(p_iX, p_iY: LongInt):Byte;//< Get Background Color of a specific character cell in the ActiveVC
Function VCGetFGC(p_iX, p_iY: LongInt):Byte;//< Get Foreground Color of a specific character cell in the ActiveVC
Function VCGetAll(p_iX, p_iY: LongInt):Word;//< return the character and color byte pair for the specific character cell in the activeVC
Procedure VCWriteCharXY(p_iX, p_iY: LongInt; p_ch:Char);//< Puts a character in the activevc at the specified x,y location.
Procedure VCWriteChar(p_Char: Char);//< writes a character at ActiveVC's current Pen's X,Y position. It then advances the pen (or cursor).
Procedure VCWrite(sa:AnsiString);//< writes a string in the ActiveVC starting at theActiveVC's current Pen's X,Y position. the pen does wrap down to the next line if it reaches the with of the ActiveVC. This routine does NOT perform a carriage return or linefeed when the text is finished.
Procedure VCWriteln;//< Performs a Carriage Return and Linefeed combination on the ActiveVC's pen
Procedure VCWriteln(sa:AnsiString);//< writes a string in the ActiveVC starting at theActiveVC's current Pen's X,Y position. the pen does wrap down to the next line if it reaches the with of the ActiveVC. This routine DOES perform a carriage return and linefeed when the text is finished.
Procedure VCWriteXY(X,Y: LongInt; sa: AnsiString) ;//< write a string at a specified location in the ActiveVC
Procedure VCWriteCTR(sa:AnsiString);//< Write a string centered on the screen in the active console using the current Pen's Y location.
Procedure VCClrEol;//< Clear vc from pen position up thru the end of the line in the active console.
Function VCGetCenterX(p_iWidth: LongInt): LongInt;//< returns the x position of the activevc that should be used to center a string p_iWidth
Function VCGetCenterY(p_iHeight: LongInt): LongInt;//< returns the y position of the activevc that should be used to center a string vertically p_iHeight high
Procedure VCFillBox(X,Y: LongInt;W,H: LongInt; ch:Char);//< fills a box in the activevc with the passed character at x,y for this width,height
Procedure VCFillBoxCTR(W,H: LongInt; ch:Char);//< Makes a centered box, width and height are W and H respectively in the ActiveVC
Procedure VCFillBoxChar(X,Y: LongInt;W,H: LongInt; ch:Char);//< fills activevc with a box at location X,Y and width and height W,H
Procedure VCFillBoxCharCTR(W,H: LongInt; ch:Char);//< This seems to do what VCFillBoxCTR does, a centered box in the ActiveVC
Procedure VCFillBoxFGC(X,Y: LongInt;W,H: LongInt; p_u1FGC:Byte);//< Fill ActiveVC with a box, where only the foreground color is changed
Procedure VCFillBoxFGCCTR(W,H: LongInt; p_u1FGC:Byte);//< Fill ActiveVC with a centered box, where only the foreground color is changed
Procedure VCFillBoxBGC(X,Y: LongInt;W,H: LongInt; p_u1BGC:Byte);//< Fill ActiveVC with a box, where only the background color is changed
Procedure VCFillBoxBGCCTR(W,H: LongInt; p_u1BGC:Byte);//< Fill ActiveVC with a centered box, where only the bacgrond color is changed
Procedure VCSetXY(p_iX, p_iY:LongInt);//< Position the activevc. Setting it "out of bounds" is ok. The animation demo with the fish is a good demonstration of this, each fish is a virtual console.
Function VCGetX: LongInt;//< Returns the X Position of the activevc
Function VCGetY: LongInt;//< Returns the Y Position of the activevc
Function VCGetPenX: LongInt;//< Returns the pen's X position in the VC
Function VCGetPenY: LongInt;//< Returns the pen's Y position in the VC
Function VCGetPenFGC: Byte;//< returns the ActiveVC Pen's foreground color
Function VCGetPenBGC: Byte;//< returns the ActiveVC Pen's background color
Function VCGetInfo: rtVCInfo;//< Populates the rtVCInfo structure with information about the ActiveVC.
Procedure VCSetInfo(p_rVCInfo: rtVCInfo);//< Allows you to populate the rtVCInfo withthe setting you want for your ActiveVC, and then calling this procedure with your loaded rtVCInfo structure will apply those values.
Procedure VCSetTransparency(p_bTransparency:Boolean);//< Set ActiveVC's transparancy on/off
Function VCGetTransparency:Boolean;//< returns activevc's current transparency on/off setting
Procedure VCSetTabsize(p_iTabsize: LongInt);//< Set how many spaces a tab represents when writing with the pen in the ActiveVC.
Function VCGetTabSize: LongInt;//< get the tabsize setting; the number of spaces a tab character represents in the ActiveVC
Procedure VCSetWrap(p_bWrap: Boolean);//< Sets the ActiveVC's wordwrap on/off
Function VCGetWrap:Boolean;//< returns the activevc's wordwrap on/off setting
//procedure VCSetScroll(p_iX, p_iY, p_iW, p_iH:LongInt);
Procedure VCScrollUp;//< Scrolls ActiveVC Text Up
Procedure VCScrollDown;//< Scrolls ActiveVC Text Down
Procedure VCScrollLeft;//< Scrolls ActiveVC Text Left
Procedure VCScrollRight;//< Scrolls ActiveVCText Right
Procedure VCSNRBoxCTR(W,H: LongInt; chfind,chreplace:Char);//< This allows you to search for (chfind) and replace (chreplace) a character in a region centered on the screen, W wide and H high.
Procedure VCSNRBox(X,Y: LongInt;W,H: LongInt; chfind,chreplace:Char);//< This allows you to search for (chfind) and replace (chreplace) a character in a region specified X,Y,W and H on the screen
Procedure VCSetOpaqueFG(p_bOpaque: Boolean);//< Set ActiveVC Opaque Foreground Setting On/Off  If On, then the foreground color of the ActiveVC will change to the color of the foreground of the VC beneath it from a z-order perspective.
Function VCGetOpaqueFG: Boolean;//< Returns the ActiveVC's Opaque Foreground Setting
Procedure VCSetOpaqueBG(p_bOpaque: Boolean);//< Set ActiveVC Opaque Background Setting On/Off  If On, then the background color of the ActiveVC will change to the color of the background of the VC beneath it from a z-order perspective.
Function VCGetOpaqueBG: Boolean;//< Returns the ActiveVC's Opaque Background Setting
Procedure VCResize(p_iNewWidth, p_iNewHeight: LongInt); //< resizes the activevc to the size you specify
Procedure VCSetVisible(p_b:Boolean);//< Sets the Visible flag on or off. Hide and show your virtual consoles at will. :)
Function VCGetVisible:Boolean;//< reports if the activevc is visible or not
Procedure VCSetTrackCursor(p_bTrackCursor: Boolean);//< This sets the ActiveVC's TrackCursor Setting. I honestly forgot what this means. It might be the "blinky" cursor thing. Better to be honest right?
Function VCGetTrackCursor:Boolean;//< returns the ActiveVC's trackcursor setting. I honestly forgot what this means. It might be the "blinky" cursor thing. Better to be honest right?
Procedure SyncAllVC;
...
Procedure VCSetScrollRegion(p_X, p_Y, p_W, p_H: LongInt);//< Set up the scroll region of the ActiveVC
Procedure VCGetScrollRegion(Var p_X, p_Y, p_W, p_H: LongInt);//< this procedure acts like a function, the parameters are passed by reference, so that you call this function, the parameters are populated with the requested information, and after the call the passed parameters contain the desired values.
Procedure VCSetScroll(p_bEnabled: Boolean);//< Set ActiveVC's Scroll Enabled Flag
Function VCGetScroll: Boolean;//< returns the ActiveVC's Scroll Enabled Flag's setting
Procedure VCSetOpaqueCh(p_bOpaque: Boolean);//< Set ActiveVC opaque character mode flag. if off, then vcs beneath, their foreground color bleeds thru, or is used to paint the ActiveVC versus the color assigned to that character. This is a visual trick, the background not being opaque is the coolest though.
Function VCGetOpaqueCH:Boolean;//< return ActiveVC's opaque ch flag setting's value
Procedure VCSetUseScrollRegionCoords(p_b: Boolean);//< You can Set scroll regions that aren't the normal "whole screen", and this is how you toggle the "custom scrolling region"
Function VCGetUseScrollRegionCoords: Boolean;//< Returns the ActiveVC's "custom scrolling region" current setting
Function VCGetZ: LongInt;//< Return Z Position of Current VC (Remember TopMost is GetVCCount result)
Function GetVCZ(p_Z: LongInt): LongInt;//< Return VC UID of Specific Z Position
Function GetZVC(p_VC: LongInt): LongInt;//< Return Z order position for Specific VC uses current Pen Position
Procedure VCWordWrap(p_iWidth,p_iHeight: LongInt; p_saMsg: AnsiString);//< Write a message on the activevc, centered, width and height that word wraps so words split at the right places, as best as it can. Toss is a super long run-on word and you will get odd results I'd think.
{}
...
//=============================================================================
  ...
  VX: LongInt; //< Virtual Screen X coord Relative to Real Video Screen
  VY: LongInt; //< Virtual Screen Y coord Relative to Real Video Screen
  VW: LongInt; //< Width of Virtual Screen
  VH: LongInt; //< Height of Virtual Screen
  {$IFDEF LESSFRAG}
    MAXMEM: LongInt; //< largest Width*height
  {$ENDIF}
  VPX: LongInt; //< Virtual Screen Pen X Coords Relative to Real Video Screen
  VPY: LongInt; //< Virtual Screen Pen Y Coords Relative to Real Video Screen
  VCX: LongInt; //< Virtual Screen Blinky Cursor Relative to Real Video Screen X
  VCY: LongInt; //< Virtual Screen Blinky Cursor Relative to Real Video Screen Y
  VCursorType: Word; //< There are different kinds of cursors you can have in the console, which is active for a given console. Although you may have any number of virtual consoles, there is only one "hardware" cursor.. (um.. that blinky thingy)
  VFGC: Byte;//< Foreground Color
  VBGC: Byte;//< Background Color
  VTabSize: Byte; //< how many spaces a tab represents
  VWrap: Boolean; //< Word Wrap Flag
  VTransparency: Boolean;
  VSX: LongInt;//< scroll window x coord
  VSY: LongInt;//< scroll window y coord
  VSW: LongInt;//< scroll window width
  VSH: LongInt;//< scroll window height
  VOpaqueFG: Boolean;//< opaque foreground flag
  VOpaqueBG: Boolean;//< opaque background flag
  VVisible:Boolean; //< visible flag
  VTrackCursor:Boolean;//< I forgot. Possibly make ..AHHH make the hardware blinky cursor thing follow the pen, or writing to the virtual console
  VScroll:Boolean;//< Scroll on/off
  VOpaqueCh: Boolean;//< opaque character on/off (a color bleed thru trick with the zorder of the virtual consoles)
  VUseScrollRegionCoords: Boolean;//< Enables the custom scrolling region.
  VBuf: ^Word; //< Buffer for the virtual console
  Public
  Procedure CreateVC(p_iVWidth, p_iVHeight:LongInt);//< create a new virtual console
  Destructor Destroy; override;//< this class is an item of a collection. This destroy would elminate this item - a Virtual Console - meta data used to take a 2d grid and make it able to draw in virtual windows, have z order etc via this library.
  Procedure Resize(p_iNewWidth, p_iNewHeight:LongInt);//< Resize the virtual console
  Function bOnscreen: Boolean; //< Returns whether or not the virtualconsole is on the screen
End;
//=============================================================================

//=============================================================================
{ }
// This is an extended JFC_XDL class used to facilitate this library
// The ZOrder is accomplished using the JFC_XDL simple bubble sort
// on the VC's Z-Order field
...
//=============================================================================
  { }
  VC: TVCXDL;//< Extended JFC_XDL used to house the virtual consoles and manage them in memory
  ActiveVC: UINT;//< Used to denote which VC the commands are being acted upon
  uMSButtons: LongInt;//< Mouse Buttons
  MG: ^UINT; //< Mouse Grid has UID of VC -  This is a dynamic "grid" the size of the "view screen". This grid is updated with the UID's of each virtual console that is "visible",so you end up with a grid that if someone clicked, you can reference and find out quickly WHICH virtual console was just clicked.
  bInvalid: Boolean; //< used to call CopyAllVCToVideo in UpdateConsole proc
  CursorX, CursorY: LongInt;//< this is where the "hardware" cursor is (blinky) in the actual view port (view screen)
  CursorType: Word;//< physical blinky type, underscore, half,full square...
  ConsoleCapabilities: Word;//< I think you need to refer to the freepascaldocumentation for the crt unit for this one.
  RVM: TVideoMode;//< I think you need to refer to the freepascaldocumentation for the crt unit for this one also.
  //  rMEvent: rtMEvent;
{ }
...
{$ENDIF}
  ...
  // Intel, Windows Special Handling
  {$IFDEF WINDOWS}
