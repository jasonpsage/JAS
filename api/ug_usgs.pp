...
var
  ...
  bOk: boolean;
begin
  {$IFDEF DEBUGMESSAGES}writeln('USGS Map Convert - Started.');{$ENDIF}
  u2IOResult:=0;
  i4FileRows:=0;
  i4FileCols:=0;
  ...
  if not bOk then
  begin
    writeln('File not found: '+saFileIn);
  end;
  {$ENDIF}
  ...
  begin
    ...
    if not bOk then
    begin
      writeln('Unable to reset file: '+saFileIn);
    end;
    {$ENDIF}
  end;
  ...
  if bOk then
  begin
    try readln(ft,saData);except on E:Exception do u2IOResult:=60000; end;//try
    u2IOResult+=u2IOResult;
    bOk:=u2IOResult=0;
    {$IFDEF DEBUGMESSAGES}
    if not bOk then
    begin
      writeln('Unable to read file: '+saFileIn);
    end;
    {$ENDIF}
  end;
  ...
  begin
    ...
    begin
      ...
      if not bOk then
      begin
        writeln('Unable to read data file: '+saFileIn);
      end;
      {$ENDIF}
      if bOk then
      begin
      ...
      try close(ft);except on E:Exception do u2IOResult:=60000; end;//try
      if bOk then
      begin
        bOk:= (i4FileCols>0) and (i4FileRows>0) and (u2IOResult=0);
        {$IFDEF DEBUGMESSAGES}
        if not bOk then
        begin
          writeln('Failure condition met:');
          writeln('FileCols: ',i4FileCols);
          writeln('FileRows: ',i4FileRows);
          writeln('u2IOResult: ',u2IOResult,' ',sIOResult(u2IOResult));
        end;
        {$ENDIF}
      end;
    end;
    ...
    begin
      ...
      assign(fin,saFileIn);
      try reset(fin);except on E:Exception do u2IOResult:=60000; end;//try
      u2IOResult+=IOResult;
      bOk:=u2IOResult=0;
      {$IFDEF DEBUGMESSAGES}
      if not bOk then
      begin
        writeln('pass2 - Unable to open: '+saFileIn);
      end;
      {$ENDIF}
    end;
    ...
    begin
      ...
      try rewrite(fout);except on E:Exception do u2IOResult:=60000; end;//try
      u2IOResult+=IOResult;
      bOk:=u2IOResult=0;
      {$IFDEF DEBUGMESSAGES}
      if not bOk then
      begin
        writeln('pass2 - Unable to write: '+saFileIn);
      end;
      {$ENDIF}
    end;
    ...
    begin
      ...
      for z:=0 to i4FileRows-1 do begin
        for x:=0 to i4FileCols-1 do begin
          try read(fin,fSingle);except on E:Exception do u2IOResult:=60000; end;//try
          u2IOResult+=ioresult;
          bOk := u2IOResult=0;
          if not bOk then
          begin
            {$IFDEF DEBUGMESSAGES}writeln('Problem reading data from file: ', saFileIn);{$ENDIF}
            break;
          end;
          bWriteIt:=false;
          ...
          begin
            ...
            begin
               try write(fout,fSingle);except on E:Exception do u2IOResult:=60000; end;//try
               u2IOResult+=ioResult;
               bOk:=u2IOResult=0;
               {$IFDEF DEBUGMESSAGES}
               if not bOk then
               begin
                 writeln('pass2 - trouble writing to: ',saFileOut);
               end;
               {$ENDIF}
            end;
          end;
    ...
    try close(fin);close(fout);except on E:Exception do u2IOResult:=60000+ioresult; end;//try
    bOk:=bOk and (u2IOResult=0);
    if bOk then
    begin
      saFileOut:=p_saOutFile_NoExtension+'.hdr';
      assign(ft,saFileOut);
      try rewrite(ft);except on E:Exception do u2IOResult:=60000; end;//try
      u2IOResult+=IOResult;
      bOk:=u2IOResult=0;
      {$IFDEF DEBUGMESSAGES}
      if not bOk then
      begin
        writeln('Unable to write to: ',saFileOut);
      end;
      {$ENDIF}
    end;
  end;
  ...
  {$ENDIF}
  try close(ft);except on E:Exception do bOk:=false; end;//try
  TK.Destroy;
  {$IFDEF DEBUGMESSAGES}
  if u2ioresult<>0 then writeln('IOResult on Exit: ',sIOResult(u2IOResult));
  {$ENDIF}
  result:=bOk;
end;
//----------------------------------------------------------------------------
...
begin
  ...
  begin
    assign(ft,saFileIn);
    try reset(ft);except on E:Exception do u2IOResult:=60000; end;//try
    u2IOResult+=IOResult;
    bOk:=u2IOResult=0;
  end;
  ...
  begin
    ...
    try close(ft);except on E:Exception do ; end;//try
    saFileIn:=p_saFilename_FloatFile_NoExtension+'.flt';
  ...
  begin                                      
    ...
    try close(fin); except on E:Exception do if u2IOResult=0 then u2IOResult:=60000; end;//try
    u2IOResult+=ioresult;
    bOk:=ioresult=0;
  ...
  begin
    ...
    try close(fin);except on E:Exception do if u2IOResult=0 then u2IOResult:=60000; end;//try
    if u2IOResult=60000 then u2IOResult+=ioresult;
    ...
    begin
      ...
      u2IOResult:=PG.u2SaveBitmap24(saFileOut,lpfnProgressBar,lpfnFormInstance);
    end;
  end;
  TK.Destroy;
  PG.Destroy;
...
begin
  ...
  begin
    ...
    u2IOResult+=IOResult;
    bOk:=IOResult=0;
    {$IFDEF DEBUGMESSAGES}if not bOk then writeln('ug_usgs - a');{$ENDIF}
  end;
  ...
  begin
    ...
    u2IOResult+=ioresult;
    bOk:=u2IOResult=0;
    {$IFDEF DEBUGMESSAGES}if not bOk then writeln('ug_usgs - b - ',u2IOResult,' ',sIOResult(u2ioResult),' saFileIn: ',saFileIn);{$ENDIF}
  end;
  ...
  begin
    ...
    u2IOResult+=ioresult;
    bOk:=u2IOResult=0;
    {$IFDEF DEBUGMESSAGES}if not bOk then writeln('ug_usgs - c');{$ENDIF}
  end;
  ...
  try close(ft); except on e:Exception do
  begin
    bOk:=false;
    {$IFDEF DEBUGMESSAGES}writeln('ug_usgs - exception:',e.message);{$ENDIF}
  end;
  end;//try
  ...
  begin
    ...
    u2IOResult+=IOResult;
    bOk:=u2IOResult=0;
    {$IFDEF DEBUGMESSAGES}if not bOk then writeln('ug_usgs - d');{$ENDIF}
  end;
  ...
  begin
    ...
    u2IOResult+=IOResult;
    bOk:=u2IOResult=0;
    {$IFDEF DEBUGMESSAGES}if not bOk then writeln('ug_usgs - e');{$ENDIF}
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        u2IOResult+=ioResult;
        bOk:=u2IOREsult=0;
        {$IFDEF DEBUGMESSAGES}if not bOk then writeln('ug_usgs - f');{$ENDIF}
        if not bOk then Break;
    ...
    try close(fin);except on E:Exception do
    begin
      if u2IOResult=0 then u2IOResult:=ioresult;
      {$IFDEF DEBUGMESSAGES}if not bOk then writeln('ug_usgs - g');{$ENDIF}
    end;
    end;//try
  end;
  ...
  begin
    ...
    u2IOResult+=IOResult;
    bOk:=u2IOresult=0;
    {$IFDEF DEBUGMESSAGES}if not bOk then writeln('ug_usgs - h');{$ENDIF}
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        u2IOResult+=ioresult;
        bOk:=u2IOResult=0;
        if not bOk then writeln('ug_usgs - i');
        if not bOk then break;
        ...
        begin
          ...
          begin
            ...
            begin
              try write(fout,u2);except on E:Exception do
              begin
                u2IOResult:=60000;
                if not bOk then writeln('ug_usgs - j');
              end;
              end;//try
              u2IOResult+=ioresult;
              bOk:=u2IOResult=0;
            ...
            begin
              ...
              begin
                try write(fOut,0);except on E:Exception do u2IOResult:=60000; end;//try
                u2IOResult+=ioresult;
                bOk:=u2IOResult=0;
                if not bOk then writeln('ug_usgs - k');
                if not bOk then break;
              end
              else
              begin
                try write(fOut,u2);except on E:Exception do u2IOResult:=60000; end;//try
                u2IOResult+=ioresult;
                bOk:=u2IOResult=0;
                if not bOk then writeln('ug_usgs - l');
                if not bOk then break;
              end;
            end;
          end;
    ...
    begin
      ...
      if not bOk then writeln('ug_usgs - m');
    end;
    end;//try
    try close(fout);except on E:Exception do
    begin
      u2IOREsult:=60000+ioresult;
      if not bOk then writeln('ug_usgs - n');
    end;
    end;//try
  end;
  TK.Destroy;
