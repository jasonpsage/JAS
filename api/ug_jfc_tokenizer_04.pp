...
Type JFC_TOKENIZER = Class(JFC_BUFFER)
//=============================================================================
  public
  {}
  saLastOne: AnsiString;
  {}
  uN: UINT;//< Used like an INDEX
  lpItem: UInt;
  uSizeToGrow: UINT;
  ...
  Function NthItem_saToken(p_u: UInt): AnsiString; //< SAFE method - Give me the "N'th" record
  ...
  function FoundNth(p_u: UINT):boolean;
...
//=============================================================================
procedure clear_rToken(var p_rToken: rtToken);
//=============================================================================
Begin
  with p_rToken do begin
    saToken:='';// use saName
...
//=============================================================================
Constructor JFC_TOKENIZER.create(p_uSize: UINT; p_uSizePerItem: UINT);
//=============================================================================
Begin
  Inherited;
  sClassname:='JFC_TOKENIZER';
  uSizeToGrow:=p_uSize * p_uSizePerItem;
  pvt_uSizePerItem:=1;
  SetDefaults;
End;
//=============================================================================
...
//=============================================================================
var p: array of ^rtToken;
Begin
  if uN>0 then
  begin
    p:=lpBuffer;
    Result:=p[uN-1]^.iQuoteID;
  end
  else
  begin
    result:=0;
...
//=============================================================================
Function JFC_TOKENIZER.NthItem_saToken(p_u: UInt):AnsiString;
//=============================================================================
var p: array of ^rtToken;
Begin
  Result:='';
  If (uItems>0) and (p_u>0) and (p_u<uSize) Then
  begin
    p:=lpBuffer;
    Result:=p[p_u-1]^.saToken;
  end;
End;
//=============================================================================
...
Begin
  ...
  begin
    ...
    begin
      saUp:=upcase(p_sa);
      for u:=1 to uItems do
      begin
        if saUp=p[u]^.saToken then
        begin
          result:=true;
          break;
        end;
...
Begin
  ...
  uN:=0;
  if uItems>0 then uN:=1;
  if (uN>0) and (uN<=uItems) then
  begin
    p:=lpbuffer;
    if not p_bCaseSensitive then
    begin
      saUp:=upcase(p_sa);
      for u:=uN to uItems do
      begin
        if saUp=p[u]^.saToken then
        begin
          result:=true;
...
{$ENDIF}
  ...
  begin
    p:=nil;
    result:=false;
    for u:=0 to uItems-1 do
    begin
      if  rtToken(p[N]).iQuoteID=p_i then begin result:=true; break; end;
    end;
  end;
{$IFDEF DIAGMSG}
...
//=============================================================================
function JFC_TOKENIZER.MoveFirst: boolean;
//=============================================================================
begin
  {$IFDEF DIAGMSG}
    writeln('BEGIN - JFC_TOKENIZER.MoveFirst');
  {$ENDIF}
    result:=(uItems>0);
    if result then uN:=1;
  {$IFDEF DIAGMSG}
    writeln('END   - JFC_TOKENIZER.MoveFirst');
  {$ENDIF}
end;
//=============================================================================

//=============================================================================
function JFC_TOKENIZER.MoveLast: boolean;
//=============================================================================
begin
  {$IFDEF DIAGMSG}
    writeln('BEGIN - JFC_TOKENIZER.MoveLast');
  {$ENDIF}
    result:=(uItems>0);
    if result then uN:=uItems;
  {$IFDEF DIAGMSG}
    writeln('END   - JFC_TOKENIZER.MoveLast');
  {$ENDIF}
end;
//=============================================================================

//=============================================================================
function JFC_TOKENIZER.MovePrevious: boolean;
//=============================================================================
begin
  {$IFDEF DIAGMSG}
    writeln('BEGIN - JFC_TOKENIZER.MovePrevious');
  {$ENDIF}
    result:=(uItems>0) and (uN>1);
    if result then uN-=1;
  {$IFDEF DIAGMSG}
    writeln('END   - JFC_TOKENIZER.MovePrevious');
  {$ENDIF}
end;
//=============================================================================

//=============================================================================
function JFC_TOKENIZER.MoveNext: boolean;
//=============================================================================
begin
  {$IFDEF DIAGMSG}
    writeln('BEGIN - JFC_TOKENIZER.MoveNext');
  {$ENDIF}
    result:=(uItems>0) and (uN<uItems);
    if result then uN+=1;
  {$IFDEF DIAGMSG}
    writeln('END   - JFC_TOKENIZER.MoveNext');
  {$ENDIF}
end;
//=============================================================================
...
//=============================================================================
function JFC_TOKENIZER.FoundNth(p_u: UINT):boolean;
//=============================================================================
begin
  {$IFDEF DIAGMSG}
    writeln('BEGIN - JFC_TOKENIZER.FoundNth');
  {$ENDIF}
    result:=(n>0) and (N<uItems*uSizePerItem) and
            (uItems>0) and (uN<uItems);
    if result then uN:=p_u;
  {$IFDEF DIAGMSG}
    writeln('END   - JFC_TOKENIZER.FoundNth');
  {$ENDIF}
end;
//=============================================================================

//=============================================================================
function JFC_TOKENIZER.AppendItem:boolean;
//=============================================================================
begin
  result:=true;
  {$IFDEF DIAGMSG}
    writeln('BEGIN - JFC_TOKENIZER.AppendItem');
  {$ENDIF}
    if N<uItems then uN+=1;
    uItems+=1;
    if (uItems * pvt_uSizePerItem)>uSize then
        uSize:=(uItems * pvt_uSizePerItem)+uSizeToGrow;
  {$IFDEF DIAGMSG}
    writeln('END   - JFC_TOKENIZER.AppendItem');
  {$ENDIF}
end;
//=============================================================================
...
//=============================================================================
function JFC_TOKENIZER.DeleteItem:boolean;
//=============================================================================
var //u: UINT;
    //uItemSize: UINT;
    //uTempItems: UINT;
    newarray: array of rtToken;
    //m: pointer;
    //N_Minus_1: uint;
    //Q: uint;
begin
  {$IFDEF DIAGMSG}
    writeln('BEGIN - JFC_TOKENIZER.DeleteItem');
  {$ENDIF}
    result:=true;
    if (uN<=uItems) and ((uItems * pvt_uSizePerItem)<=uSize) then
    begin
      getmem(pointer(newarray),uSize * sizeof(rtToken));
      //for u:= N to length(lpbuffer) do
      //begin
        //pointer(newarray[u].saToken):=rtToken(lpBuffer[u]).saToken;
        //newarray[u].iQuoteID:=rtToken(lpBuffer[u]).uQuoteID;
        //newarray[u].i8User:=rtToken(lpBuffer[u]).i8User;
        //uItemItems+=1;
      //end;
    end;
    result:=true;
  {$IFDEF DIAGMSG}
    writeln('END   - JFC_TOKENIZER.DeleteItem');
  {$ENDIF}
end;
//=============================================================================
...
//=============================================================================
//Function JFC_TOKENIZER.TFoundBinary(
//  p_lpWhat: pointer;
//  p_uSize: UInt;
//  p_lpOffset: pointer;
//  p_bStartAtBegin: Boolean;
...
//    End;
//  End;
  //riteln('FoundBinary...(iItems:',uItems,')');
//End;
//=============================================================================
