...
Begin
  ...
  Begin
    ...
    try
      ...
      Writeln(f,'aQuote Array length or count = ', sizeof(aQuote));
      If length(aQuote)>0 Then
      Begin
        for u2:=0 to length(aQuote)-1 do
        begin
          Writeln(f,'Quote Pair ',u2+1,': 1=First. start (Length=',
            length(aQuote[u2].sStart), ') >',
            saHumanReadable(aQuote[u2].sStart),'< end (Length=',
            length(aQuote[u2].sEnd), ') >',
            saHumanReadable(aQuote[u2].sEnd),'<');
        end;
      End;
      Writeln(f);
    ...
    Begin
      ...
      Begin//not in quotemode
        ...
        Begin
          ...
          for u2:=0 to length(aQuote)-1 do
          begin
            If ipsalength-ibp>=length(aQuote[u2].sStart) Then
            Begin
              If ((not bCaseSensitive) and 
                     (copy(saUp,iBP,length(aQuote[u2].sStart))=
                     upcase(aQuote[u2].sStart)))
                 OR
                 (bCaseSensitive and     
                     (copy(p_sa,iBP,length(aQuote[u2].sStart))=
                     aQuote[u2].sStart))
              Then
              begin
                bGotStartQuote:=True;
          ...
          Begin
            ...
            {$IFDEF DIAGMSG}
            writeln('JFC_TOKENIZER.Tokenizemore - aQuote[iQuoteMode-1].sStart: ',aQuote[iQuoteMode-1].sStart);
            {$ENDIF}

            Item_i8Pos:=iBP+1;
            iBP+=length(aQuote[u2].sStart);
            ...
            {$IFDEF DIAGMSG}
            writeln('JFC_TOKENIZER.Tokenizemore - Is End Quote Nothing? >',aQuote[iQuoteMode-1].sEnd,'<');
            {$ENDIF}
            // If End Quote is '' (nothing) then
            // Make the current token the rest of the line and bail
            If not (aQuote[iQuoteMode-1].sEnd='') Then
            Else 
            Begin
              iQuoteMode:=0;
      ...
      Begin//quotemode
        ...
        Begin // Multi Char Quote System
          ...
          If not ((ipsalength-ibp>=length(aQuote[iQuoteMode-1].sEnd)) and (
              
              ((not bCaseSensitive) and 
                   (copy(saUp,iBP,length(aQuote[iQuoteMode-1].sEnd))=
                   upcase(aQuote[iQuoteMode-1].sEnd)))
               OR
               (bCaseSensitive and     
                   (copy(p_sa,iBP,length(aQuote[iQuoteMode-1].sEnd))=
                   aQuote[iQuoteMode-1].sEnd))
            )) Then 
          Begin
            Item_saToken:=Item_saToken+p_sa[ibp];
            ibp+=1;
          ...
          Begin
            ...
            {$ENDIF}
            iBP+=length(aQuote[iQuoteMode-1].sEnd);
            iQuoteMode:=0;
            {$IFDEF DIAGMSG}
...
{$ENDIF}
  setlength(aQuote,length(aQuote)+1);
  aQuote[length(aQuote)-1].sStart:=p_sStart;
  aQuote[length(aQuote)-1].sEnd:=p_sEnd;
