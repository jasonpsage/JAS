...
Begin
  ...
  With rCGIIN Do Begin
    ...
    uPostContentLength:=0; // Length of posted Content.
    uEnvVarCount:=envcount;
  ...
  If (rCGIIN.uEnvVarCount>=cn_MaxEnvWarnThreshold) Then
  Begin 
    If (rCGIIN.uEnvVarCount<cn_MaxEnvVariables) Then
    Begin
      {$IFNDEF CGITHIN_TIGHTCODE}
      JLog(MAC_SEVERITY_WARN,13, 'CGI Environment variables past hardcoded ' +
        ...
        ' This invocation had ' + inttostr(rCGIIN.uEnvVarCount) + '.',
        SOURCEFILE);
      {$ENDIF}
      Result:=13;
    End
    ...
    Begin
      ...
      JLog(MAC_SEVERITY_ERROR,14, 'Maximum CGI Environment variables EXCEEDED ' +
        ...
        SOURCEFILE);
      {$ENDIF}
      rCGIIN.uEnvVarCount:=cn_MaxEnvVariables;
  ...
  For u2:=0 To rCGIIN.uEnvVarCount-1 Do
  Begin
    sa:=envstr(u2);
    eq:=pos('=',sa);
    rCGIIN.arNVPair[u2].saName:=copy(sa,1,eq-1);
    rCGIIN.arNVPair[u2].saValue:=copy(sa,eq+1,length(sa)-eq);
    ...
    // variables.
    If (rCGIIN.arNVPair[u2].saName=csCGI_QUERY_STRING) then
    begin
      rCGIIN.arNVPair[u2].saValue:=saSNRStr(rCGIIN.arNVPair[u2].saValue,'%%','%',true);
    end;
    
    {$IFDEF CGIIN_DIAGNOSTIC_LOG}
    try writeln(f,t,' "',sa,'" equal pos=',eq,' NAME: "',rCGIIN.arNVPair[u2].saName,'" Value: "',rCGIIN.arNVPair[u2].saValue,'"'); except on E:Exception do ; end;//try
    {$ENDIF}
    If (rCGIIN.arNVPair[u2].saName=csCGI_REQUEST_METHOD) Then
    Begin
      rCGIIN.iRequestMethodIndex:=u2;
    End;       
    If (rCGIIN.arNVPair[u2].saName=csCGI_CONTENT_TYPE) Then
    Begin
      rCGIIN.iPostContentTypeIndex:=u2;
    End;       
    If (rCGIIN.arNVPair[u2].saName=csCGI_CONTENT_LENGTH) Then
    Begin
      //Writeln('Content Length String:'+rCGIIN.arNVPair[t].saValue);
      rCGIIN.uPostContentLength:=uVal(rCGIIN.arNVPair[u2].saValue);
  ...
  Begin // avoid crash cuz env limit maxed out - possibilty low but...
    ...
    Begin
      ...
      Begin
        If rCGIIN.uPostContentLength>0 Then
        Begin
          cc:=0;
          ...
          rCGIIN.saPostData:='';
          While cc<rCGIIN.uPostContentLength Do Begin
            cc+=1; //rite(cc,' ');
