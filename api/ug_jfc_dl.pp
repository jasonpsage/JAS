...
//=============================================================================
  ...
  //---------------------------------------------------------------------------
  {}
  lpOwner: pointer; //< DESCENDANT DEFINED - can be handy
  iType  : INT; //< Value is DESCENDANT DEFINED, use as you like. 32bit or 64bit - depends on endian
  uItems: UInt; // < 32bit or 64bit - depends on endian
  uN: UInt; // 32bit or 64bit - depends on endian
  lpFirst: pointer; 
  lpLast: pointer;
  bCopy: Boolean;
  ...
  Property  ListCount: UInt read UItems;
  //---------------------------------------------------------------------------
  {}
  ...
  Function  FoundNth(p_uN: UInt):Boolean;
  //---------------------------------------------------------------------------
  ...
  Function SortBinary(
    p_uSize: UInt;  //< Size of BINARY data "FIELD"
...
Begin
  ...
  If bDebug Then Begin
    JLog(MAC_SEVERITY_DEBUG,630,'JFC_DL.pvt_createtask classname:'+sClassName+' lpItem:'+inttostr(UINT(lpitem)),MAC_SOURCEFILE);
  End;
  {$ENDIF}
End;
//=============================================================================
...
Begin
  If bCopy Then
  Begin
    self.lpFirst:=JFC_DL(p_lpSameTypeOfClass).lpFirst;
    self.lpLast:=JFC_DL(p_lpSameTypeOfClass).lpLast;
    self.lpItem:=JFC_DL(p_lpSameTypeOfClass).lpItem;
    self.uItems:=JFC_DL(p_lpSameTypeOfClass).uItems;
    self.uN:=JFC_DL(p_lpSameTypeOfClass).uN;
  End;
End;
//=============================================================================
...
Begin
  ...
  {$IFDEF DEBUG_G_JFC_DL}
  If not Result Then JLog(MAC_SEVERITY_DEBUG,691,'JFC_DL.AppendItem Failed- classname:' + sClassName, MAC_SOURCEFILE);
  {$ENDIF}
End;
//=============================================================================
...
Begin
  ...
  {$IFDEF DEBUG_G_JFC_DL}
  If not Result Then JLog(MAC_SEVERITY_DEBUG,709,'JFC_DL.AppendItem_lpPtr Failed- classname:' + sClassName, MAC_SOURCEFILE);
  {$ENDIF}
End;
//=============================================================================
...
Begin
  ...
  {$IFDEF DEBUG_G_JFC_DL}
  If not Result Then JLog(MAC_SEVERITY_DEBUG,726,'JFC_DL.InsertItem_lpPtr Failed- classname:' + sClassName, MAC_SOURCEFILE);
  {$ENDIF}
...
Begin
  ...
  {$IFDEF DEBUG_G_JFC_DL}
  If not Result Then JLog(MAC_SEVERITY_DEBUG,744,'JFC_DL.InsertItem_lpPtr Failed- classname:' + sClassName, MAC_SOURCEFILE);
  {$ENDIF}
...
Begin
  If (uItems>0) and (not bCopy) Then
  Begin
    Result:=lpItem;
    uItems-=1; //NOTE: TODO: Debugging this code (in C++) suggested moving
  ...
  Begin
    Result:=nil;
    uN:=uItems;// just make sure N is iNN Sync :)
  End;
End;
//=============================================================================
...
Begin
  {$IFDEF DEBUG_G_JFC_DL}
  If bDebug Then Begin
    JLog(MAC_SEVERITY_DEBUG,945,'JFC_DL - DeleteAll - BEGIN - CLASS:'+sClassName, MAC_SOURCEFILE);
  End;
  {$ENDIF}
    
  Result:=(not bCopy);
  If Result and (uItems>0) Then Begin
    While Result and (uItems>0) Do Begin
      Result:=DeleteItem;
    End;
  End;
  ...
  {$IFDEF DEBUG_G_JFC_DL}
  If bDebug Then Begin
    JLog(MAC_SEVERITY_DEBUG,958,'JFC_DL - DeleteAll - END   - CLASS:'+sClassName, MAC_SOURCEFILE);
  End;
  {$ENDIF}
End;
//=============================================================================
...
//=============================================================================
Function JFC_DL.FoundNth(p_uN: UInt): Boolean;
//=============================================================================
Begin
  Result:=False;
  If (uItems>0) and (p_uN>0) and (uItems>=p_uN) Then
  Begin
    If uN<>p_uN Then
    Begin
      ...
      Begin
        ...
        If (p_uN-uN) < (uItems-p_uN) Then
        Begin
          // from here forward
          While (uN<>p_uN) and (not EOL) Do Begin MoveNext; End;
        End
        Else
        Begin
         // from end backward
          MoveLast;
          While (uN<>p_uN) and (not BOL) Do Begin MovePrevious; End;
        End;
      End
      Else
      Begin
...
//    begin
    ...
    Begin
      if(p_uSize mod 4)=0 then
      begin
        uCalcSize:=p_uSize div 4;
        //MoveFirst;
        If not p_buseptr Then
        Begin
          repeat
            lpHuntMe:=pointer(UINT(lpItem)+UINT(p_lpOffset));
            bGotIT:=(comparedword(p_lpWhat^, lpHuntMe^,uCalcSize)=0);
        ...
        Else Begin
          ...
          repeat
            ...
            //riteln('lpHuntMe:',cardinal(lpHuntMe),' bGotIt:',bGotIt,' N:',N, ' lpItem:',uint(lpITem));
          Until bGotIT OR (not MoveNext);
      ...
      begin
        if(p_uSize mod 2)=0 then
        begin
          uCalcSize:=p_uSize div 2;
          //MoveFirst;
          If not p_buseptr Then
          Begin
            repeat
              lpHuntMe:=pointer(UINT(lpItem)+UINT(p_lpOffset));
              bGotIT:=(compareword(p_lpWhat^, lpHuntMe^,uCalcSize)=0);
            Until bGotIT OR (not MoveNext);
          End
          Else Begin
            //riteln('foundbinary Repeat Loop...BEGIN');
        ...
        begin
          ...
          Begin
            repeat
              lpHuntMe:=pointer(UINT(lpItem)+UINT(p_lpOffset));
              bGotIT:=(comparebyte(p_lpWhat^, lpHuntMe^,p_uSize)=0);
            Until bGotIT OR (not MoveNext);
          End
          Else Begin
            //riteln('foundbinary Repeat Loop...BEGIN');
            repeat
              lpHuntMe:=pointer(UINT(JFC_DLITEM(lpItem).lpPtr)+UINT(p_lpOffset));
              //riteln('comparebyte call... p_uSize:',p_uSize);
              bGotIT:=(comparebyte(p_lpWhat^, lpHuntMe^,p_uSize)=0);
              //riteln('Back from comparebyte call... p_uSize:',p_uSize);
              //riteln('lpHuntMe:',cardinal(lpHuntMe),' bGotIt:',bGotIt,' N:',N, ' lpItem:',cardinal(lpITem));
            Until bGotIT OR (not MoveNext);
            //riteln('foundbinary Repeat Loop...DONE');
          End;
...
//    end;
    ...
    End;
  End;
  //riteln('Leaving JFC_DL.FoundBinary...(iItems:',uItems,')');
End;
//=============================================================================
    ...
    Begin
      ...
      Else Begin
        repeat
          ...
          Else
            saSearchIn:=AnsiString(pointer(UINT(JFC_DLITEM(lpItem).lpPtr)+UINT(p_lpOffset))^);
          bGotIt:=(saSearchFor=saSearchIn);
        Until bGotIT OR (not MoveNext);
      End;
    End;
...
Begin
  ...
  // it could, like an ISAM system, be made to have arrays used as indexes...
  // in memory database enhancement :)
  Result:=(not bCopy) and (uItems>0);
  If uItems>1 Then Begin// Do Actual Sort
    If not p_busePtr Then Begin
      lpBookMark:=lpItem;
      //uT1:=1;
      //while uT1<= iItems do begin
      For uT1:=1 To uItems Do Begin
        MoveFirst;
        uT2:=1;
        While uT2<=uItems-1 Do Begin
      ...
      If p_bAscending Then Begin
        If not p_bCaseSensitive Then Begin
          ...
          For uT1:=1 To uItems Do Begin
            ...
            uT2:=1;
            While uT2<= uItems-1 Do Begin
            //for uT2:=1 to iItems-1 do begin
              lpItem1:=lpItem;
              lpItem2:=JFC_DLITEM(lpITem).lpNext;
        ...
        End Else Begin
          ...
          For uT1:=1 To uItems Do Begin
            ...
            uT2:=1;
            While uT2<= uItems-1 Do Begin
            //for uT2:=1 to uItems-1 do begin
              lpItem1:=lpItem;
              lpItem2:=JFC_DLITEM(lpITem).lpNext;
