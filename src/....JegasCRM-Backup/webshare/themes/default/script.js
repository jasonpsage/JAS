...
function artGTranslateFix() {
	var menus = artGetElementsByClassName("jas-menu", document, "ul");
...
function artMenuIE6Setup() {
	...
	var aTmp = artGetElementsByClassName("jas-menu", document, "ul");
...
function artLayoutIESetup() {
    ...
    if (!isIE) return;
    var q = artGetElementsByClassName("jas-content-layout", document, "div");
    ...
    for (var i = 0; i < q.length; i++) {
        ...
        for (var p = 0; p < l_childs.length; p++) {
            var l_ch = l_childs[p];
            if ((String(l_ch.tagName).toLowerCase() == "div") && artHasClass(l_ch, "jas-content-layout-row")) {
                r = l_ch;
                break;
            }
        }
        ...
        for (var o = 0; o < r_childs.length; o++) {
            var r_ch = r_childs[o];
            if ((String(r_ch.tagName).toLowerCase() == "div") && artHasClass(r_ch, "jas-layout-cell")) {
                c.push(r_ch);
            }
        }
...
function artAddVMenuSeparators() {
	...
	var menus = artGetElementsByClassName("jas-vmenublock", document, "div");
...
/* begin VMenuItem */
function artVMenu() {
    var menus = artGetElementsByClassName("jas-vmenu", document, "ul");
    for (var k = 0; k < menus.length; k++) {
        var vmenu = menus[k];
...
function artButtonsSetupJsHover(className) {
	...
	for (var j = 0; j < tags.length; j++){
		...
		for (var i = 0; i < buttons.length; i++) {
			...
			if (!artHasClass(button.parentNode, 'jas-button-wrapper')) {
				if (!artHasClass(button, 'jas-button')) button.className += ' jas-button';
...
artLoadEvent.add(function() { artButtonsSetupJsHover("jas-button"); });
/* end Button */
