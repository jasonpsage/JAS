...
begin
  ...
  writeln(saRepeatchar('+',79));
  writeln(saRepeatchar('+',79));
  writeln(csCRLF+'DIVE: ',p_saDir,' FULLPATH: ',p_saFullPath);
  bOk:=true;
  result:=false;
  ...
  begin
    ...
    writeln('bDive - Directory Change Successful:'+syesNo(bOk)+' Server work Dir: '+FC.GetCurrentDir);
    if not bOk then
    begin
    ...
    begin
      ...
      begin
        ...
        //  begin
        //    riteln('Server: '+FC.FTPList.Items[u4].Filename);
        ...
        begin
          repeat
            sa:=RightStr(JFC_TOKENITEM(TK.lpItem).saToken,length(JFC_TOKENITEM(TK.lpItem).saToken)-62);//da snip
    ...
    begin
      repeat
        ...
        begin
          ...
          writeln('begot? bOk:'+sYesNo(bOK)+' File: '+FC.DirectFilename);
        ...
        writeln('FileXDL.Item_saName: ',FileXDL.Item_saName, ' Directory: ',sYesNo(FileXDL.Item_i8User=1));
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              bHandleD:=true;
              //riteln(FC.GetCurrentDir);
              //riteln(FC.FullResult.text);
            end;
          ...
          begin
            ...
            //f bOk then riteln('Switch to DIR: ',tk.Item_saToken)
          end else
          ...
          begin
            ...
            begin
              ...
              begin
                bHandled:=bSpecialCommand('DELETE', true);
                if not bHandled then writeln('DELETE FAIL - Recurse: '+FC.FullResult.text);
                bHandled:=FC.ChangeToParentDir;
                if not bHandled then writeln('DELETE FAIL - ChangeToParentDir: '+FC.FullResult.text);
              end;

              if bHandled then
              begin
                bHandled:=FC.DeleteDir(TK.Item_saToken);
                if not bHandled then writeln('Delete Directory Failed: '+FC.FullResult.text);
              end;
            end;
          end;
        end;
...
begin
  ...
  begin
    writeln;
    writeln('Usage:    jegasftp HOST PORT [USERNAME] [PASSWORD]');
    writeln('Examples: jegasftp 127.0.0.1 21');
    writeln('          jegasftp 127.0.0.1 21 john mypassword');
