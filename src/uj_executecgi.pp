...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102318, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  //result:='200 OK';
  p_Context.CGIENV.iHTTPResponse:=cnHTTP_RESPONSE_200;//ok
  BytesRead:=0;n:=0;
  ...
  begin
    ...
    begin
      sa:='';
      repeat
        sa+=p_Context.CGIENV.CKYIN.Item_saName+'='+p_Context.CGIENV.CKYIN.Item_saValue+'; ';
      until not p_Context.CGIENV.CKYIN.Movenext;
      p_Context.PROCESS.Environment.Add(csCGI_HTTP_COOKIE+'='+ sa );
    
      //sa:='';
      //repeat
      //  p_Context.PROCESS.Environment.Add(csCGI_HTTP_COOKIE+'='+p_Context.CGIENV.CKYIN.Item_saName+'='+p_Context.CGIENV.CKYIN.Item_saValue);
      //  JAS_LOG(p_Context,cnLog_Debug, 201009141722,'COOKIE for Process Environment: '+csCGI_HTTP_COOKIE+'='+p_Context.CGIENV.CKYIN.Item_saName+'='+p_Context.CGIENV.CKYIN.Item_saValue,'',SOURCEFILE);
      //until not p_Context.CGIENV.CKYIN.Movenext;

    end;

    //JAS_Log(p_Context, cnLog_Debug, 201009141721,'COOKIE DUMP IN EXECUTE CGI: '+p_Context.CGIENV.CKYIN.saHtmlTable,'',SOURCEFILE);
    ...
    p_Context.PROCESS.Environment.Add(csCGI_CONTENT_LENGTH+'='+inttostr(p_Context.CGIENV.iPostContentLength));    
    ...
    p_Context.PROCESS.Environment.Add('SERVER_SIGNATURE=Jegas Application Server/0.0.4 '+p_Context.saRequestedHost+' Port:'+p_Context.saServerPort);
    p_Context.PROCESS.Environment.Add('SERVER_NAME='+p_Context.saRequestedHost);
    p_Context.PROCESS.Environment.Add('SERVER_ADDR='+p_Context.saServerIP);
    p_Context.PROCESS.Environment.Add('SERVER_PORT='+p_Context.saServerPort);
    ...
    Begin
      ...
      Begin
        ...
        begin
          ...
          TK.saSeparators:=csCR;
          TK.saWhiteSpace:=csCRLF;
          ...
          { This code block seeks a cookie out put with path like path=/ and makes sure it has the correct path on it.. path=/admin/
          if(TK.MoveFirst) then
          begin
            repeat
              if saLeftStr(TK.Item_saToken,12)='Set-Cookie: ' then
              begin
                if saRightStr(TK.Item_saToken,6)='path=/' then 
                begin
                  jasprintln('******************************GOT MESSED COOKIE');
                  ...
                  begin
                    ...
                    with p_Context do begin 
                      ...
                      jasprintln('saRemoteIP:'+saRemoteIP);
                      jasprintln('saRemotePort:'+saREmotePort);
                      jasprintln('saServerIP:'+saServerIP);
                      jasprintln('saServerPort:'+saServerPort);
                      jasprintln('i4BytesRead:'+inttostr(i4bytesread));
                      jasprintln('i4BytesSent:'+inttostr(i4bytessent));
                      jasprintln('bBlackListed:'+satruefalse(bBlacklisted));
                      jasprintln('bWhiteListed:'+satruefalse(bWhitelisted));
                      jasprintln('bAccessDenied:'+satruefalse(bAccessdenied));
                      jasprintln('dtRequested:'+saFormatDateTime(csDATETIMEFORMAT,dtRequested));
                      jasprintln('saRequestedHost:'+saREquestedhost);
                      jasprintln('saRequestedHostRootDir:'+sarequestedhostrootdir); 
                      jasprintln('saRequestMethod:'+sarequestmethod);
                      jasprintln('saRequestType:'+sarequesttype);
                      jasprintln('saRequestedFile:'+sarequestedfile);
                      jasprintln('saOriginallyREquestedFile:'+saoriginallyrequestedfile);
                      jasprintln('saRequestedName:'+sarequestedname);
                      jasprintln('saRequestedDir:'+sarequesteddir);
                      jasprintln('saFileSought:'+safilesought);
                      jasprintln('saQueryString:'+saquerystring);
                      jasprintln('i4ErrorCode:'+inttostr(i4errorcode));
                      jasprintln('saLogFile:'+salogfile);
                    end;//endwith
                  end;
                end;
              end; 
        ...
        begin      
          p_Context.CGIENV.iHTTPResponse:=cnHTTP_RESPONSE_302;
        end;
      end;
  ...
  begin
    p_Context.saOut:=p_Context.CGIENV.saHTTPServerError500(200903211212,'Server Error - ExecuteCGI','Server Error', p_Context.saMimeType);
  end;
  ...
  if p_Context.CGIENV.iHTTPResponse=0 then p_Context.CGIENV.iHTTPResponse:=cnHTTP_RESPONSE_200; 
