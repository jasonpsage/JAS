...
//=============================================================================
  ...
  begin
    writeln('bSaveAsText');
    bGood:=0=pg.bSaveAsText('test.txt');
    if not bGood then
    begin
      writeln('bSaveAsText failed!');
    end;
  end;

  if bGood then
  begin
    writeln('bSaveBitmap24');
    bGood:=0=pg.bSaveBitmap24('test.bmp',nil,nil);

    if not bGood then
    begin
      writeln('bSaveBitMap24 failed!');
    end;
  end;

  if bGood then
  begin
    writeln('bLoadBitmap24');
    bGood:=0=pg.bLoadBitmap24('test.bmp');
    if not bGood then
    begin
      writeln('bLoadBitMap24 failed!');
    end;
  end;

  if bGood then
  begin
    writeln('bSaveBitmap24 (after test load)');
    bGood:=0=pg.bSaveBitmap24('test.bmp',nil,nil);
    if not bGood then
    begin
      writeln('bSaveBitMap24 failed!');
