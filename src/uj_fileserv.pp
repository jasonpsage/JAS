...
Function saHtmlRipper(p_Context: TCONTEXT; p_u8Caller: uint64): AnsiString;
//=============================================================================
{}
...
Var 
  ...
  saTemp: ansistring;
  saGetFileCaller: ansistring;// this holds an error id but is only used as text so we allow it to live
...
//----------------------------
                        ...
                        Begin
                          {$IFDEF DEBUGLOGBEGINEND}
                            DebugIn(sTHIS_ROUTINE_NAME+'.SendOutTheData Caller:'+inttostr(p_u8Caller),SourceFile);
                          {$ENDIF}
                          ...
                          End;                           
                          {$IFDEF DEBUGLOGBEGINEND}
                            DebugOut(sTHIS_ROUTINE_NAME+'.SendOutTheData Caller:'+inttostr(p_u8Caller),SourceFile);
                          {$ENDIF}
...
{$IFDEF ROUTINENAMES}
  sTHIS_ROUTINE_NAME:='Function saGetPage(p_Context:tContext;p_saArea: AnsiString;p_saPage: AnsiString;p_saSection: AnsiString;'+
    'p_bSectionOnly:Boolean;p_saCustomSection: AnsiString;p_u8Caller: uint64): AnsiString;';
{$ENDIF}
{$IFDEF DEBUGLOGBEGINEND}
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102321, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  JASPrintLn('p_bSectionOnly:'+sTRueFalse(p_bSectionOnly));
  ...
  saLoadedFileData:='';
  saData:='';
  saGetFileCaller:=inttostr(p_u8Caller);//saGetFileCaller:=saGetFileCaller;//shutup compiler - frowned practice LOL
  saFileName:=p_saArea;

  //house keeping measure
  p_Context.saRequestedDir:=saAddSlash(p_Context.saRequestedDir);//only adds one if one not there already
  garJVHost[p_Context.u2VHost].VHost_TemplateDir:=saAddSlash(garJVHost[p_Context.u2VHost].VHost_TemplateDir);
  garJVHost[p_Context.u2VHost].VHost_WebShareDir:=saAddSlash(garJVHost[p_Context.u2VHost].VHost_WebShareDir);
  ...
  Else 
  Begin
    saFilename:=garJVHost[p_Context.u2VHost].VHost_DefaultArea+csJASFileExt;
    {$IFDEF DIAGMSG}
  ...
  Begin
    ...
    Begin
      //------- LOCATE METHOD
      {$IFDEF DIAGMSG}
      JASPrintln('saGetFile - Attempting saJASLocateTemplate on:'+saFilename+' Lang:'+p_Context.sLang+' Theme: '+p_Context.sTheme+' GetFileCaller:'+saGetFileCaller);
      {$ENDIF}
      saTemp:=saJASLocateTemplate(
        saFilename,
        p_Context.sLang,
        p_Context.sTheme,
        ...
        garJVHost[p_Context.u2VHost].VHost_WebShareDir
      );
      ...
      begin
        JAS_LOG(p_Context, cnLog_Warn, 201204142128, 'saGetPage - Unable to find Area file: '+p_saArea+' Caller: '+ saGetFileCaller,'',SOURCEFILE);
      end;
      {$IFDEF DIAGMSG}
      ...
      begin
        ...
        {$ENDIF}
        saFilename:=garJVHost[p_Context.u2VHost].VHost_TemplateDir+ p_Context.sLang+DOSSLASH+safilename;
        {$IFDEF DIAGMSG}
  ...
  {$IFDEF DIAGMSG}
    ...
    JASPrintln('saGetPage - Filename: '+ saFilename +' Exists: '+sTRueFalse( fileexists(saFileName))+' GetFileCaller:'+saGetFileCaller);
  {$ENDIF}
  ...
  Begin
    ...
    TK1.AppendQuotePair(csHTMLRipperStart,csHTMLRipperEnd);
    ...
    Begin
      ...
      Begin
        ...
        If not FileExists(saFilename) Then
        begin
          ...
          begin
            ...
            Begin
              // Make FileName Work With Default HTMLFILES directory.
              //----------------------------------------------------
              saFilename:=garJVHost[p_Context.u2VHost].VHost_TemplateDir+grJASConfig.sDefaultLanguage+DOSSLASH+p_saPage + csJASFileExt;
              //JAS_LOG(p_Context, nLog_Warn, 201204142129, 'saGetPage - Unable to find Page file: '+p_saPage+' Filename: '+saFilename +' Caller: '+ inttostr(p_u8Caller),'',SOURCEFILE);
        ...
        {$IFDEF DIAGMSG}
        JASPrintln('1 saGetPage - Page Filename: '+safilename+' Exists: '+sTrueFalse(fileexists(saFilename)));
        {$ENDIF}
        ...
        begin
          JASPrintln('saFilename did not load: '+saFilename);
        {$ENDIF}
        End;
        ...
        Begin
          ...
          //  begin
              {$IFDEF DIAGMSG}
              JASPrintln('Begin Error Page time because the page was not found anywhere, not eventhe default one.');
              {$ENDIF}
              JAS_Log(p_Context,cnLog_Error,200610122306,'Unable to load a needed template file to deliver this page section.','file: ' +saFilename + ' and unable to find insert section from default template file.',SOURCEFILE);
              p_Context.saPage:='ErrorNo       : '+inttostr(p_Context.u8ErrNo)+csCRLF+
                                'Error Message : '+p_Context.saErrMsg+csCRLF+
                                'Please report this error to your administrator if it persists.'+csCRLF+csCRLF;
              p_Context.CGIENV.Header_PlainText(garJVHost[p_Context.u2VHost].VHost_ServerIdent);
              {$IFDEF DIAGMSG}
              JASPrintln('End Error Page time. came back, moving on');
              {$ENDIF}
          //  end;
          ////--end   test
        End;
  ...
  Else 
  Begin
    JAS_Log(p_Context,cnLog_Error,200609272025,'Unable to LOAD HTML template file.','file: ' +saFilename + ' Reason:'+sIOResult(u2IOResult),SOURCEFILE);
  ...
  {$IFDEF DIAGMSG}
  JASPrintln('saGetPAge-----------------------END (bSuccess:'+sTRueFalse(bSuccess)+')');
  {$ENDIF}
...
{$IFDEF ROUTINENAMES}
  sTHIS_ROUTINE_NAME:='Function saGetPage(p_Context:TCONTEXT;p_saArea: AnsiString;p_saPage: AnsiString;p_saSection: AnsiString;'+
    'p_bSectionOnly: Boolean;p_u8Caller: uint64): AnsiString;';
{$ENDIF}
{$IFDEF DEBUGLOGBEGINEND}
  DebugIn(sTHIS_ROUTINE_NAME,SourceFile);
{$ENDIF}
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102327, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  Result:=saGetPage(p_Context,p_saArea, p_saPage, p_saSection, p_bSectionOnly,'',p_u8Caller);
...
{$IFDEF ROUTINENAMES}var sTHIS_ROUTINE_NAME: String;{$ENDIF}
Begin
{$IFDEF ROUTINENAMES}sTHIS_ROUTINE_NAME:='saHtmlRipper(p_Context:TCONTEXT; p_u8Caller: uint64): AnsiString;';{$ENDIF}
{$IFDEF DEBUGLOGBEGINEND}
  DebugIn(sTHIS_ROUTINE_NAME,SourceFile);
{$ENDIF}
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102330, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  JASPrintLn('  ENTRY: upcase(p_Context.sFileSoughtExt):'+upcase(p_Context.sFileSoughtExt));
  {$ENDIF}
  ...
  begin
    ...
    begin
      if upcase(p_Context.sFileSoughtExt)='JAS' then
      begin
        p_Context.saHTMLRIPPER_Area:=p_Context.sFileNameOnlyNoExt;
      end
      else
      begin
        p_Context.saHTMLRIPPER_Area:=garJVHost[p_Context.u2VHost].VHost_DefaultArea;
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      p_Context.saHTMLRIPPER_Page:=garJVHost[p_Context.u2VHost].Vhost_DefaultPage;
    end;
  end;
  ...
  begin
    ...
    begin
      p_Context.saHTMLRIPPER_Section:=garJVHost[p_Context.u2VHost].VHost_DefaultSection;
    end
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102336, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  if bOk then
  begin
    JNote_JNote_UID:=u8Val(p_Context.CgiEnv.DataIn.Get_saValue('UID'));
    bDecodeURI:=bVal(p_Context.CgiEnv.DataIn.Get_saValue('decodeuri'));
    bOk:=JNote_JNote_UID>0;
    if not bOk then
    begin
      JAS_Log(
        p_Context,
        ...
        'JNote_JNote_UID: ' +inttostr(JNote_JNote_UID)+
  ...
  p_Context.sMimeType:='text/plain';
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102238, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  JNote_JNote_UID:=u8Val(p_Context.CgiEnv.DataIn.Get_saValue('UID'));
  p_Context.saPage:=saGetPage(p_Context, '','sys_page_mindmaps','',true,'',201101132054);
  p_Context.PageSNRXDL.AppendItem_saName_N_saValue('[@JNote_JNote_UID@]',inttostr(JNote_JNote_UID));
  p_Context.sMimeType:='text/html';
