...
{}
// JASAPIXML Function
// This is a JASAPI XML function that just produces a somewhat LARGE response
// just to cause a purposed heavy TRANSMISSION of data using the JASAPIXML
...
//=============================================================================
{}
// returns cookie JASDISPLAYHEADERS set to "on" or "off" based on value set.
// this ajax cookie stuff is more work than necessary as javascript could be
// used to handle such bits - but this approach allows for some neat
// things if they are stealing bandwidth etc during use to much.
procedure JASAPI_SetDisplayHeaders(p_Context: TCONTEXT);
//=============================================================================
{}
// returns cookie JASDISPLAYQUICKLINKS set to "on" or "off" based on value set.
// this ajax cookie stuff is more work than necessary as javascript could be
// used to handle such bits - but this approach allows for some neat
// things if they are stealing bandwidth etc during use to much.
...
//=============================================================================
{}
// returns cookie JASICONTHEME set to one of the icon themes' names.
// this ajax cookie stuff is more work than necessary as javascript could be
// used to handle such bits - but this approach allows for some neat
// things if they are stealing bandwidth etc during use to much.
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102760, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  p_Context.saFileNameOnlyNoExt:=lowercase(p_Context.saFileNameOnlyNoExt);
  ...
  if (p_Context.saFileNameOnlyNoExt='validatesession')then 
  begin 
    jasapi_validatesession(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFileNameOnlyNoExt='createsession')then 
  begin 
    jasapi_createsession(p_Context);
    goto done_jasapi;
  end;
  
  if (p_Context.saFileNameOnlyNoExt='removesession')then 
  begin 
    jasapi_removesession(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFileNameOnlyNoExt='bigtest')then 
  begin 
    jasapi_bigtest(p_Context); 
    goto done_jasapi; 
  end;

  if (p_Context.saFileNameOnlyNoExt='renderquicklinks')then
  begin
    jasapi_renderquicklinks(p_Context);
    goto done_jasapi;
  end;

  if(p_Context.saFilenameOnlyNoExt='settheme') then
  begin
    JASAPI_SetTheme(p_Context);
    goto done_jasapi;
  end;

  if(p_Context.saFilenameOnlyNoExt='setdisplayheaders') then
  begin
    JASAPI_SetDisplayHeaders(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='setdisplayquicklinks') then
  begin
    JASAPI_SetDisplayQuickLinks(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='seticontheme') then
  begin
    JASAPI_SetIconTheme(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='maketable') then
  begin
    JASAPI_MakeTable(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='tableexist') then
  begin
    JASAPI_TableExist(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='columnexist') then
  begin
    JASAPI_ColumnExist(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='makecolumn') then
  begin
  ...
  if (p_Context.saFilenameOnlyNoExt='rowexist') then
  begin
    JASAPI_RowExist(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='email') then
  begin
    JASAPI_Email(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='filtersavedef') then
  begin
    JASAPI_FilterSaveDef(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='movequicklink') then
  begin
    JASAPI_MoveQuickLink(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFileNameOnlyNoExt='register')then
  begin
  ...
  if (p_Context.saFilenameOnlyNoExt='copyscreen') then
  begin
    JASAPI_CopyScreen(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='newconanddb') then
  begin
    JASAPI_NewConAndDB(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='dbmasterutility') then
  begin
    JASAPI_DBMasterUtility(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='setdefaultsecuritysettings') then
  begin
    JASAPI_SetDefaultSecuritySettings(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='createvirtualhost') then
  begin
    JASAPI_CreateVirtualHost(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='doesuserhavejet') then
  begin
    JASAPI_DoesUserHaveJet(p_Context);
    goto done_jasapi;
  end;
  
  if (p_Context.saFilenameOnlyNoExt='jetbilling') then
  begin
    JASAPI_JetBilling(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='getvalue') then
  begin
    JASAPI_GetValue(p_Context);
    goto done_jasapi;
  end;

  if (p_Context.saFilenameOnlyNoExt='userprefhelp') then
  begin
    JASAPI_UserPrefHelp(p_Context);
    goto done_jasapi;
  end;

  //p_Context.CGIENV.iHTTPResponse:=422;// 422 - unprocessable entity
  p_Context.CGIENV.iHTTPResponse:=200;// Force Bad Call through as xml error response
  p_Context.bErrorCondition:=true;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102676, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  p_Context.XML.AppendItem_saName_n_saValue('validatesession',trim(saTrueFalse(bJAS_ValidateSession(p_Context))));
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102679, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  p_Context.XML.AppendItem_saName_N_saValue('createsession',saStr(p_Context.iSessionResultCode));
  ...
  begin
    ...
    p_Context.XML.AppendItem_saName_N_saValue('sessionid',p_Context.rJSession.JSess_JSession_UID);
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102682, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  p_Context.XML.AppendItem_saName_N_saValue('removesession',trim(saTrueFalse(bOk)));
...
//=============================================================================
// This sends a rather large (8meg I think) response to verify SIZE doesn't
// impact system ability to stream entire contents. 
procedure jasapi_bigtest(p_Context: TCONTEXT);
//=============================================================================
var
  bOk: boolean;
...
{$IFDEF ROUTINENAMES}  sTHIS_ROUTINE_NAME: String; {$ENDIF}
Begin
{$IFDEF ROUTINENAMES}  sTHIS_ROUTINE_NAME:='jasapi_bigtest(p_Context: TCONTEXT);'; {$ENDIF}

{$IFDEF DEBUGLOGBEGINEND}
  DebugIn(sTHIS_ROUTINE_NAME,SourceFile);
{$ENDIF}
{$IFDEF DEBUGTHREADBEGINEND}p_Context.JThread.DBIN(201203102684,sTHIS_ROUTINE_NAME,SOURCEFILE);{$ENDIF}
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102685, sTHIS_ROUTINE_NAME);{$ENDIF}

  bOk:=p_Context.bSessionValid;
  if not bOk then
  begin
    JAS_LOG(p_Context, cnLog_Error, 201204140348, 'jasapi_removesession - Invalid Session','',SOURCEFILE);
  end;

  if bOk then
  begin
    with p_Context do begin
      p_Context.XML.AppendItem_saName_n_saValue('bigtest','Woo Hoo B^)');

      if(p_Context.CGIENV.CKYIN.MoveFirst)then
      begin
        xdlnode:=JFC_XDL.Create;
        p_Context.XML.Item_lpPtr:=xdlnode;
        for i:=0 to 65000 do
        begin
...
{$IFDEF DEBUGLOGBEGINEND}
  DebugOut(sTHIS_ROUTINE_NAME,SourceFile);
{$ENDIF}
{$IFDEF DEBUGTHREADBEGINEND}p_Context.JThread.DBOUT(201203102686,sTHIS_ROUTINE_NAME,SOURCEFILE);{$ENDIF}
end;
//=============================================================================
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102691, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    bOk:=bJas_Save_jTestOne(p_Context,DBC,rJTestOne,false,false);
    if not bOk then
    begin
  ...
  p_Context.XML.AppendItem_saName_N_saValue('result',lowercase(satruefalse(bOk)));
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102694, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  if 'OFF'<>upcase(trim(p_Context.CGIENV.CKYIN.get_savalue('JASDISPLAYQUICKLINKS'))) then
  begin
    DBC:=p_Context.VHDBC;
    rs:=JADO_RECORDSET.Create;
    
    bJMail:=(0<DBC.u8GetRowCount('jmail','JMail_CreatedBy_JUser_ID='+DBC.saDBMSUIntScrub(p_Context.rJUser.JUser_JUser_UID)+' and '+
      'JMail_From_User_ID<>'+DBC.saDBMSUIntScrub(p_Context.rJUser.JUser_JUser_UID)+' AND ((JMail_Deleted_b=false)or(JMail_Deleted_b is null))',201506171747));
    ...
    begin
      saQry:=
	      ...
	      '  JQLnk_Private_b '+
	      'FROM jquicklink '+
	      'WHERE JQLnk_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+' and ';
    ...
    if bOk then
    begin
      saData:='<ul class="jas-quicklinks">';
      if p_Context.bSessionValid then
      begin
        saData+='<li><a title="Home" href="." target="_blank"><img src="[@JASDIRICONTHEME@]22/places/user-home.png"></a></li>';
      end;
      saData+='<li><a target="_blank" href="[@ALIAS@].?Action=JAS_Calendar" title="Calendar" >'+
          '<img src="[@JASDIRICONTHEME@]22/apps/accessories-date.png" /></a></li>';
          
      if bJMail then  
      begin
        saData+='<li><a target="_blank" href="[@ALIAS@].?screen=jmail+Search" title="JMail" >'+
          '<img src="[@JASDIRICONTHEME@]22/actions/mail.png" /></a></li>';
      end;
      ...
      begin 
        repeat
          ...
          begin 
            ...
            sIcon:=saSNRStr(sIcon, '[@JASDIRICONTHEME@]',grJASConfig.saWebShareAlias+'img/icon/themes/'+p_Context.sIconTheme+'/');
            sIcon:=saSNRStr(sIcon, '<? echo $JASDIRICONTHEME; ?>',grJASConfig.saWebShareAlias+'img/icon/themes/'+p_Context.sIconTheme+'/');
            sIcon:=saSNRStr(sIcon, '[@JASDIRICON@]',grJASConfig.saWebShareAlias+'img/icon/');
            sIcon:=saSNRStr(sIcon, '<? echo $JASDIRICON; ?>',grJASConfig.saWebShareAlias+'img/icon/');
         
            if NOT (bVal(rs.fields.Get_saValue('JQLnk_RenderAsUniqueDiv_b')))then
            begin
              saData+='<li><a target="_blank" href="'+rs.fields.Get_saValue('JQLnk_URL')+'" '+
                      ' title="'+saHtmlScrub(rs.fields.get_saValue('JQLnk_Name'))+'" >'+sIcon+'</a></li>';
            end
            else
            begin
              saData+='<li><div id="QLDIV'+rs.fields.Get_saValue('JQLnk_JQuickLink_UID')+'">'+
                '<a id="QLANCHOR'+rs.fields.Get_saValue('JQLnk_JQuickLink_UID')+'">'+sIcon+'</a></div></li>';
            end;
          end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102697, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  saData:='';
  saThemeRequested:=trim(p_Context.CGIENV.DataIn.Get_saValue('name'));
  saQry:='THCOL_Deleted_b=false and THCOL_Name='''+trim(saThemeRequested)+'''';  
  bOk:=  1 = p_Context.VHDBC.u8GetRowCount('jthemecolor',saQry,201506211121);
  if not bOk then
  begin
    p_Context.bErrorCondition:=true;
    p_Context.u8ErrNo:=201506211122;
    p_Context.saErrMsg:='SetTheme JASAPI called without a Theme Color that '+
      'actually exists. E.g. ?JASAPI=settheme&name=default  Here is the query '+
      'that did not return the desired result count of 1: '+saQry;
    p_Context.saErrMsgMoreInfo:='';
  end;
  
  if bOk then
  begin
    saData+='<result>true</result>';
    p_Context.CGIENV.AddCookie('JASCOLORTHEME',saThemeRequested,p_Context.CGIENV.saRequestURIWithoutParam,dtAddMonths(now,1),false);
    JASPrintln('jasapi_settheme: '+p_Context.CGIENV.saRequestURIWithoutparam);
  end;
  p_Context.XML.AppendItem_saName_N_saValue('jastheme',saData);
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102700, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  //  p_bSecure: Boolean
  //);
  if(bVal(p_Context.CGIENV.DataIn.Get_saValue('value')))then
  begin
    p_Context.CGIENV.AddCookie('JASDISPLAYHEADERS','on',p_Context.CGIENV.saRequestURIWithoutparam,dtAddMonths(now,1),false);
  end
  else
  begin
    p_Context.CGIENV.AddCookie('JASDISPLAYHEADERS','off',p_Context.CGIENV.saRequestURIWithoutParam,dtAddMonths(now,1),false);
  end;
  p_Context.XML.AppendItem_saName_N_saValue('result','true');
  if p_Context.CGIENV.iHTTPResponse=0 then
  begin
    p_Context.CGIENV.iHTTPResponse:=200;// Not setting this will cause an error.
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102703, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  if(bVal(p_Context.CGIENV.DataIn.Get_saValue('value')))then
  begin
    p_Context.CGIENV.AddCookie('JASDISPLAYQUICKLINKS','on',p_Context.CGIENV.saRequestURIWithoutParam,dtAddMonths(now,1),false);
  end
  else
  begin
    p_Context.CGIENV.AddCookie('JASDISPLAYQUICKLINKS','off',p_Context.CGIENV.saRequestURIWithoutParam,dtAddMonths(now,1),false);
  end;
  p_Context.XML.AppendItem_saName_N_saValue('result','true');
  if p_Context.CGIENV.iHTTPResponse=0 then
  begin
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102706, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  //);
  bOk:=true;
  s:=upcase(p_Context.CGIENV.DataIn.Get_saValue('name'));
  sIconTheme:='';
  if(s='CRYSTALCLEAR') then
    sIconTheme:='CrystalClear' else
      if(s='NUOVEXT') then
        sIconTheme:='NuoveXT' else
          if(s='NUVOLA') then
            sIconTheme:='Nuvola' else
              if(s='VISTAINSPIRATE') then
                 sIconTheme:='VistaInspirate'
              else
                bOk:=false;
  if bOk then
  begin
    p_Context.CGIENV.AddCookie('JASICONTHEME',sIcontheme,p_Context.CGIENV.saRequestURIWithoutParam,dtAddMonths(now,1),false);
    p_Context.XML.AppendItem_saName_N_saValue('result','true');
  end
  else
  begin
    p_Context.XML.AppendItem_saName_N_saValue('result','false');
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102712, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    if not bOk then
    begin
      JAS_Log(p_Context,cnLog_ERROR, 201110101808, 'JAS_MakeTable - Unable to load JTable record. UID: ' + rJTable.JTabl_JTable_UID, '',SOURCEFILE);
    end;
  end;
  ...
  begin
    ...
    begin
      JAS_Log(p_Context,cnLog_Error, 201110101809, 'JAS_MakeTable - Unable to connect to database. '+
        'DBCON came back null for JTable UID: ' + rJTable.JTabl_JTable_UID+
        ' JDConnection ID: '+rJTable.JTabl_JDConnection_ID, '',SOURCEFILE);
    end;
  end;

  sPKey:=TGT.saGetPkeyColumn(pointer(TGT),rJTable.JTabl_Name,201210020202);
  if sPKey='' then sPKey:=rJTable.JTabl_ColumnPrefix+'_'+saProper(rJTable.JTabl_Name)+'_UID';
  ...
  begin
    ...
    begin
      saDDL:='CREATE TABLE '+TGT.saDBMSEncloseObjectName(rJTable.JTabl_Name)+' '+
        '('+TGT.saDBMSEncloseObjectName(sPKey)+' BIGINT UNSIGNED NOT NULL, PRIMARY KEY ('+TGT.saDBMSEncloseObjectName(sPKey)+'))';
  ...
  begin
    saQry:=
      ...
      'from jcolumn '+
      'where '+
      ' JColu_JTable_ID='+rJTable.JTabl_JTable_UID+' and '+
  ...
  begin
    repeat
      ...
      //begin
        bOk:=bJAS_MakeColumn(p_Context, rJColumn.JColu_JColumn_UID);
        if not bOk then
        begin
          JAS_Log(p_Context,cnLog_Error, 201110111218, 'JAS_MakeTable - JAS_MakeColumn Failed '+
            ' DBCON: '+TGT.ID+' JTabl_JTable_UID:'+rJTable.JTabl_JTable_UID+
            ' JTabl_Name: ' + rJTable.JTabl_Name+
            ' JColu_JColumn_UID: '+rJColumn.JColu_JColumn_UID+
  ...
  begin
    ...
    //begin
      ...
      with rJColumn do begin
        ...
        JColu_JDType_ID               := inttostr(cnJDType_u8);
        JColu_AllowNulls_b              := '0';
        JColu_DefaultValue            := '0';
        JColu_PrimaryKey_b              := '1';
        JColu_JAS_b                   := '0';
        JColu_JCaption_ID             := 'NULL';
        JColu_DefinedSize_u             := '8';
        JColu_NumericScale_u            := '1';
        JColu_Precision_u               := '0';
        JColu_Boolean_b                 := '0';
        JColu_JAS_Key_b                := '1';
        JColu_AutoIncrement_b      := '0';
        JColu_LUF_Value               := 'NULL';
        JColu_LD_CaptionRules_b         := 'NULL';
        JColu_JDConnection_ID         := TGT.ID;
        JColu_Desc                    := 'NULL';
        JColu_CreatedBy_JUser_ID:=p_Context.rJSession.JSess_JUser_ID;
        JColu_Created_DT:=saFormatDateTime(csDATETIMEFORMAT,DT);
        JColu_ModifiedBy_JUser_ID:=p_Context.rJSession.JSess_JUser_ID;
        JColu_Modified_DT:=saFormatDateTime(csDATETIMEFORMAT,DT);
        JColu_DeletedBy_JUser_ID      :='NULL';
        JColu_Deleted_DT              :='NULL';
        JColu_Deleted_b               :='0';
        JAS_Row_b                     :='0';
      end;//with
      bOk:=bJAS_Save_JColumn(p_Context, DBC, rJColumn, false, false);
      if not bOk then
      begin
        JASPrintln('FAILED making jcolumn record');
        JAS_Log(P_Context,cnLog_Error,201110110459,'JAS_MakeTable - Trouble with bSave_JColumn','JColu_JColumn_UID:'+rJColumn.JColu_JColumn_UID,SOURCEFILE);
      end;
  ...
  begin
    ...
    begin
      ...
      with rJColumn do begin
        ...
        JColu_JTable_ID               := rJTable.JTabl_JTable_UID;
        JColu_JDType_ID               := inttostr(cnJDType_u8);
        JColu_AllowNulls_b              := '1';
        JColu_DefaultValue            := '0';
        JColu_PrimaryKey_b              := '0';
        JColu_JAS_b                   := '0';
        JColu_JCaption_ID             := 'NULL';
        JColu_DefinedSize_u             := '8';
        JColu_NumericScale_u            := '1';
  ...
  begin
    ...
    begin
      ...
      with rJColumn do begin
        ...
        JColu_Name                    := sField;
        JColu_JTable_ID               := rJTable.JTabl_JTable_UID;
        JColu_JDType_ID               := inttostr(cnJDType_dt);
        JColu_AllowNulls_b              := '1';
        JColu_DefaultValue            := '0';
        JColu_PrimaryKey_b              := '0';
        JColu_JAS_b                   := '0';
  ...
  begin
    ...
    begin
      ...
      with rJColumn do begin
        ...
        JColu_Name                    := sField;
        JColu_JTable_ID               := rJTable.JTabl_JTable_UID;
        JColu_JDType_ID               := inttostr(cnJDType_b);
        JColu_AllowNulls_b              := '1';
        JColu_DefaultValue            := '0';
        JColu_PrimaryKey_b              := '0';
        JColu_JAS_b                   := '0';
  ...
  if bOk then
  begin
    clear_JSecPerm(rJSecPerm);rJSecPerm.JSPrm_Name:=rJTable.JTabl_Name+' Add';
    if 0=DBC.u8GetRowCount('jsecperm','JSPrm_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+' AND '+
      'JSPrm_Name='+DBC.saDBMSSCrub(rJSecPerm.JSPrm_Name),201506171748) then
    begin
      bOk:=bJAS_Save_JSecPerm(p_Context, DBC, rJSecPerm, false,false);
      if not bOk then
      begin
        JAS_LOG(p_Context, cnLog_Error, 201205030052,'Trouble saving new permission: ' +rJSecPerm.JSPrm_Name,'',SOURCEFILE);
      end;
    end;
  end;
  ...
  if bOk then
  begin
    clear_JSecPerm(rJSecPerm);rJSecPerm.JSPrm_Name:=rJTable.JTabl_Name+' Edit';
    if 0=DBC.u8GetRowCount('jsecperm','JSPrm_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+' AND '+
      'JSPrm_Name='+DBC.saDBMSSCrub(rJSecPerm.JSPrm_Name),201506171749) then
    begin
      bOk:=bJAS_Save_JSecPerm(p_Context, DBC, rJSecPerm, false,false);
      if not bOk then
      begin
        JAS_LOG(p_Context, cnLog_Error, 201205030053,'Trouble saving new permission: ' +rJSecPerm.JSPrm_Name,'',SOURCEFILE);
      end;
    end;
  end;
  ...
  if bOk then
  begin
    clear_JSecPerm(rJSecPerm);rJSecPerm.JSPrm_Name:=rJTable.JTabl_Name+' View';
    if 0=DBC.u8GetRowCount('jsecperm','JSPrm_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+' AND '+
      'JSPrm_Name='+DBC.saDBMSSCrub(rJSecPerm.JSPrm_Name),201506171748) then
    begin
      bOk:=bJAS_Save_JSecPerm(p_Context, DBC, rJSecPerm, false,false);
      if not bOk then
      begin
        JAS_LOG(p_Context, cnLog_Error, 201205030054,'Trouble saving new permission: ' +rJSecPerm.JSPrm_Name,'',SOURCEFILE);
      end;
    end;
  end;
  ...
  if bOk then
  begin
    clear_JSecPerm(rJSecPerm);rJSecPerm.JSPrm_Name:=rJTable.JTabl_Name+' Delete';
    if 0=DBC.u8GetRowCount('jsecperm','JSPrm_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+' AND '+
      'JSPrm_Name='+DBC.saDBMSSCrub(rJSecPerm.JSPrm_Name),201506171749) then
    begin
      bOk:=bJAS_Save_JSecPerm(p_Context, DBC, rJSecPerm, false,false);
      if not bOk then
      begin
        JAS_LOG(p_Context, cnLog_Error, 201205030055,'Trouble saving new permission: ' +rJSecPerm.JSPrm_Name,'',SOURCEFILE);
      end;
    end;
  end;
  ...
  if bOk then
  begin
    clear_JSecPerm(rJSecPerm);rJSecPerm.JSPrm_Name:=rJTable.JTabl_Name+' Modify';
    if 0=DBC.u8GetRowCount('jsecperm','JSPrm_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+' AND '+
      'JSPrm_Name='+DBC.saDBMSSCrub(rJSecPerm.JSPrm_Name),201506171750) then
    begin
      bOk:=bJAS_Save_JSecPerm(p_Context, DBC, rJSecPerm, false,false);
      if not bOk then
      begin
        JAS_LOG(p_Context, cnLog_Error, 201205030056,'Trouble saving new permission: ' +rJSecPerm.JSPrm_Name,'',SOURCEFILE);
      end;
    end;
  end;
  ...
  if bOk then
  begin
    clear_JSecPerm(rJSecPerm);rJSecPerm.JSPrm_Name:=rJTable.JTabl_Name+' Export';
    if 0=DBC.u8GetRowCount('jsecperm','JSPrm_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+' AND '+
      'JSPrm_Name='+DBC.saDBMSSCrub(rJSecPerm.JSPrm_Name),201506171751) then
    begin
      bOk:=bJAS_Save_JSecPerm(p_Context, DBC, rJSecPerm, false,false);
      if not bOk then
      begin
        JAS_LOG(p_Context, cnLog_Error, 201205030057,'Trouble saving new permission: ' +rJSecPerm.JSPrm_Name,'',SOURCEFILE);
      end;
    end;
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102715, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  p_Context.XML.AppendItem_saName_N_saValue('result',lowercase(saTrueFalse(bOk)));
  //p_Context.XML.AppendItem_saName_N_saValue('ok',lowercase(saTrueFalse(bOk)));
  //p_Context.XML.AppendItem_saName_N_saValue('TableExist',lowercase(saTrueFalse(bTableExists)));
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102718, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    TGT:=nil;
    TGT:=p_Context.DBCON(i8Val(rJTable.JTabl_JDConnection_ID));
    bOk:=TGT<>nil;
    if not bOk then
    begin
      ...
      p_Context.saErrMsg:='JAS_TableExist - Unable to connect to database. '+
        'DBCON came back null for JTable UID: ' + rJTable.JTabl_JTable_UID+
        ' JDConnection ID: '+rJTable.JTabl_JDConnection_ID;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102721, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    if not bOk then
    begin
      JAS_LOG(p_Context, cnLog_Error, 201204140409, 'Not authorized. "Database Maintenance" permission required for this operation.','',SOURCEFILE);
    end;
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102724, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  DBC:=p_COntext.VHDBC;
  clear_jcolumn(rJColumn);
  rJColumn.JColu_JColumn_UID:=p_JColu_JColumn_UID;
  bOk:=bJAS_Load_JColumn(p_Context, DBC, rJColumn, false);
  if not bOk then
  begin
    p_Context.u8ErrNo:=201110082230;
    p_Context.saErrMsg:='JAS_ColumnExist - Unable to load JColumn record. UID: ' + rJColumn.JColu_JColumn_UID;
    JAS_Log(p_Context,cnLog_FATAL, p_Context.u8ErrNo, p_Context.saErrMsg, '', SOURCEFILE);
  end;
  ...
  begin
    ...
    begin
      ...
      p_Context.saErrMsg:='JAS_ColumnExist - Unable to load JTable record. UID: ' + rJTable.JTabl_JTable_UID;
      JAS_Log(p_Context,cnLog_FATAL, p_Context.u8ErrNo, p_Context.saErrMsg, '', SOURCEFILE);
    end;
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102730, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    p_Context.saErrMsg:='JAS_MakeColumn - Unable to load JColumn record. UID: ' + rJColumn.JColu_JColumn_UID;
    JAS_Log(p_Context,cnLog_FATAL, p_Context.u8ErrNo, p_Context.saErrMsg, '', SOURCEFILE);
  end;
  ...
  begin
    ...
    begin
      ...
      p_Context.saErrMsg:='JAS_MakeColumn - Unable to load JTable record. UID: ' + rJTable.JTabl_JTable_UID;
      JAS_Log(p_Context,cnLog_FATAL, p_Context.u8ErrNo, p_Context.saErrMsg, '', SOURCEFILE);
    end;
  end;
  ...
  begin
    ...
    begin
      // USE ALTER COLUMN
      saQRY+='MODIFY ' ;//+TGT.saDBMSEncloseObjectName(rJColumn.JColu_Name) +' ';
    end
    else
    begin
      // USE CREATE COLUMN
      saQRY+='ADD ';// +TGT.saDBMSEncloseObjectName(rJColumn.JColu_Name) +' ';
    end;
    ...
    saQry+=JADO.saDDLField(
      rJColumn.JColu_Name,
      TGT.u2DBMSID,
      u2Val(rJColumn.JColu_JDType_ID),
      u8Val(rJColumn.JColu_DefinedSize_u),
      i4Val(rJColumn.JColu_NumericScale_u),
      i4Val(rJColumn.JColu_Precision_u),
      saDefault,
      bVal(rJColumn.JColu_AllowNulls_b),
      bVal(rJColumn.JColu_AutoIncrement_b),
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102736, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      p_Context.saErrMsg:='JASAPI_RowExist - Unable to load JTable record. UID: ' + rJTable.JTabl_JTable_UID;
      JAS_Log(p_Context,cnLog_FATAL, p_Context.u8ErrNo, p_Context.saErrMsg, '', SOURCEFILE);
    end;
  end;
  ...
  begin
    saQry:='select JColu_Name '+
      'from jcolumn '+
      'where JColu_JTable_ID='+rJTable.JTabl_JTable_UID+' and '+
      'JColu_JAS_Key_b='+DBC.saDBMSBoolScrub(true)+' and '+
      'JColu_Deleted_b<>'+DBC.saDBMSBoolScrub(true);
    ...
    begin
      ...
      p_Context.saErrMsg:='JAS_RowExist - Unable to locate JegasKey column '+
        'row for JTable UID: ' + rJTable.JTabl_JTable_UID+
        ' JDConnection ID: '+rJTable.JTabl_JDConnection_ID;
  ...
  begin
    saQry:='select '+saUIDColumn+' from '+rJTable.JTabl_Name + ' where '+
      ...
      DBC.saGetColumnPrefix(TGT.ID,rJTable.JTabl_Name)+'_Deleted_b<>'+
      TGT.saDBMSBoolScrub(true);
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201204070000, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    rJFiltersaveDef.JFtSD_JFilterSave_ID:=p_Context.CGIENV.DataIn.Get_saValue('filtersave');
    saQry:='select JFtSa_JBlok_ID from jfiltersave '+
      'WHERE JFtSa_JFilterSave_UID='+DBC.saDBMSUIntScrub(rJFilterSaveDef.JFtSD_JFilterSave_ID)+' AND '+
            'JFtSa_Deleted_b<>'+DBC.saDBMSBoolScrub(true);
    ...
    if not rs.eol then
    begin
      rJFilterSaveDef.JFtSD_JBlok_ID:=rs.fields.Get_saValue('JFtSa_JBlok_ID');
      saQry:='JBlok_JBlok_UID='+DBC.saDBMSUIntScrub(rJFilterSaveDef.JFtSD_JBlok_ID)+' AND '+
        'JBlok_Deleted_b<>'+DBC.saDBMSBoolScrub(true);
      if DBC.u8GetRowCount('jblok',saQry,201506171752)>0 then
      begin
        rs.close;
        saQry:='select JFtSD_JFilterSaveDef_UID from jfiltersavedef '+
          ' WHERE JFtSD_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+' AND '+
          ' JFtSD_JBlok_ID='+DBC.saDBMSUIntScrub(rJFilterSaveDef.JFtSD_JBlok_ID)+' AND '+
          ' JFtSD_CreatedBy_JUser_ID='+DBC.saDBMSUIntScrub(p_Context.rJUser.JUser_JUser_UID);
        ...
        begin
          repeat
            bOk:=bJAS_DeleteRecord(p_Context,DBC,'jfiltersavedef', rs.fields.Get_saValue('JFtSD_JFilterSaveDef_UID'));
          until (not bOk) or (not rs.movenext);
        end;
        rs.close;
        ...
        if bOk then
        begin
          bOk:=bJAS_Save_JFilterSaveDef(p_Context, DBC, rJFilterSaveDef, false, false);
          if not bOk then
          begin
  ...
  p_Context.XML.AppendItem_saName_N_saValue('result',lowercase( trim(saTrueFalse( bok))));
  p_Context.XML.AppendItem_saName_N_saValue('filtersave',rJFilterSaveDef.JFtSD_JFilterSave_ID);
  p_Context.iErrorReportingMode:=cnSYS_INFO_MODE_SECURE;// Prevent debug info being appended
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201204162114, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  if bOk then
  begin
    saQry:='SELECT JQLnk_JQuickLink_UID FROM jquicklink WHERE JQLnk_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+' AND ';
    if bVal(rs.fields.Get_saValue('private')) then
    begin
      saQry+='JQLnk_Owner_JUser_ID='+DBC.saDBMSUIntScrub(p_Context.rJUser.JUser_JUser_UID)+' AND JQLnk_Private_b='+DBC.saDBMSBoolScrub(true);
    end
    else
    begin
      saQry+='JQLnk_Private_b='+DBC.saDBMSBoolScrub(false);
    end;
    saQry+=' ORDER BY JQLnk_Position_u';
  ...
  begin
    ...
    begin
      u4Pos:=1;
      repeat
        saQry:='update jquicklink set JQLnk_Position_u='+DBC.saDBMSUintScrub(u4Pos)+
          ' WHERE JQLnk_JQuickLink_UID='+DBC.saDBMSUIntScrub(rs.fields.Get_saValue('JQLnk_JQuickLink_UID'));
  ...
  begin
    saQry:='select JQLnk_Position_u from jquicklink where JQLnk_JQuickLink_UID='+DBC.saDBMSUIntScrub(p_Context.CGIENV.DataIn.Get_saValue('UID'));
  ...
  begin
    saQry:='update jquicklink set JQLnk_Position_u='+DBC.saDBMSUintScrub(u4Pos)+
      ' WHERE JQLnk_Position_u='+DBC.saDBMSUIntScrub(u4NewPos)+' AND ';
    if bVal(rs.fields.Get_saValue('private')) then
    begin
      saQry+='JQLnk_Owner_JUser_ID='+DBC.saDBMSUIntScrub(p_Context.rJUser.JUser_JUser_UID)+' AND JQLnk_Private_b='+DBC.saDBMSBoolScrub(true);
    end
  ...
  begin
    saQry:='update jquicklink set JQLnk_Position_u='+DBC.saDBMSUintScrub(u4NewPos)+
      ' WHERE JQLnk_JQuickLink_UID='+DBC.saDBMSUIntScrub(p_Context.CGIENV.DataIn.Get_saValue('UID'));
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102356, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  if garJVHostLight[p_Context.i4VHost].bAllowRegistration then
  begin
    ...
    with p_Context do begin
      ...
      if bOk then
      begin
        bOk:=(DBC.u8GetRowCount('jperson','((JPers_Home_Email='+DBC.saDBMSScrub(saEmail) + ') or (JPers_Work_Email='+DBC.saDBMSScrub(saEmail) + ')) AND '+
      ...
      // Name Exists ------------------------------------------------------------
      if DBC.u8GetRowCount('jperson',
        'UCASE(TRIM(JPers_NameFirst))='+DBC.saDBMSScrub(Upcase(saFirstname)) + ' and ' +
        'UCASE(TRIM(JPers_NameLast))='+DBC.saDBMSScrub(Upcase(saLastname)) + ' AND ' +
        '((JPers_Deleted_b=false) or (JPers_Deleted IS NULL))',201506171754) >0 then
      begin
        bOk:=false;
      ...
      // Cellphone --------------------------------------------------------------
      if garJVHostLight[p_Context.i4VHost].bRegistrationReqCellPhone then
      begin
      ...
      // Birthday ---------------------------------------------------------------
      if garJVHostLight[p_Context.i4VHost].bRegistrationReqBirthday then
      begin
        bBirthdayOk:=true;
    ...
    begin
      ...
      bOk:=bJAS_Save_JPerson(p_Context, DBC, rJPerson, false, false);
      if not bOk then
      begin
    ...
    begin
      ...
      with rJUser do begin
        ...
        JUser_Password               :=saxorstring(trim(saNewPassword1),grJASConfig.u1passwordkey);
        JUser_JPerson_ID             :=u8Val(rJPerson.JPers_JPerson_UID);
      ...
      bOk:=bJAS_Save_JUser(p_Context, DBC, rJUser, false, false);
      if not bOk then
      begin
        saMsg:=p_Context.saErrMsg;
        JAS_Log(p_Context,cnLog_Error,201208180243,saMsg,'',SOURCEFILE,DBC);
        saQry:='DELETE FROM jperson WHERE JPers_JPerson_UID='+DBC.saDBMSUIntScrub(rJPerson.JPers_JPerson_UID);
    ...
    begin
      ...
      bOk:=bJAS_Save_JJobQ(p_Context, DBC, rJJobQ, false,false);
      if not bOk then
      begin
        ...
        saQry:='DELETE FROM juser WHERE JUser_JUser_UID='+DBC.saDBMSUIntScrub(rJUser.JUser_JUser_UID);
  ...
  //  saTrueFalse(bJAS_RecordLockValid(p_Context,JAS.ID, 'juser',rJUser.JUser_JUser_UID,'0')
  //),'',SOURCEFILE);
  JASPRintln('register xml - done');
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102640, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  JASPrintln('JAS_JScreenCopy - BEGIN');

  DBC:=p_Context.VHDBC;
  rs:=JADO_RECORDSET.Create;
  rs2:=JADO_RECORDSET.Create;

  JASPrintln('JAS_JScreenCopy - Check Session');

  // TEST ERROR---------- 
  //bOk:=false;
  //saErrCode:='1';
  //saErrMsg:='JAS_JScreenCopy - Invalid Session. You are not logged in.'+' '+SOURCEFILE;
  // TEST ERROR----------
  ...
  JASPrintln('JAS_JScreenCopy - Check If Admin');
  If bOk Then
  Begin
  ...
  JASPrintln('JAS_JScreenCopy - CLear Screen Records in mem');
  clear_jscreen(Src_rJScreen);
  clear_jscreen(Dest_rJScreen);

  JASPrintln('JAS_JScreenCopy - Check if have valid param');
  if bOk then
  begin
  ...
  begin
    ...
    if bOk then
    begin
      Src_rJScreen.JScrn_JScreen_UID:=p_Context.CGIENV.DataIn.Item_saValue;
    end
    else
    begin
  ...
  JASPrintln('JAS_JScreenCopy - Load Src Screen');
  if bOk then
  begin
    bOk:=bJAS_Load_JScreen(p_Context, DBC, Src_rJScreen, false);
    if not bOk then
    begin
      JAS_LOG(p_Context,cnLog_Error,201009121621,'Screen Copy - Trouble loading original screen record.',
        'bJAS_Load_JScreen - Src_rJScreen.JScrn_JScreen_UID: '+Src_rJScreen.JScrn_JScreen_UID,SOURCEFILE);
    end;
  end;

  JASPrintln('JAS_JScreenCopy - Check for potential dupe');
  if bOk then
  begin
    saQry:='select count(*) as MyCount from jscreen where '+
      '(UPPER(JScrn_Name)='+DBC.saDBMSScrub(upcase(Dest_rJScreen.JScrn_Name))+') and '+
      '((JScrn_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(JScrn_Deleted_b IS NULL))';
  ...
  if bOk then
  begin
    JASPrintln('JAS_JScreenCopy - Populate Dest Screen Record');
    ...
    bOk:=bJAS_Save_JScreen(p_Context, DBC, Dest_rJScreen, false,false);
    if not bOk then
    begin
  ...
  if bok then
  begin
    JASPrintln('JAS_JScreenCopy - Query JBlok');
    saQry:='select JBlok_JBlok_UID, JBlok_JScreen_ID from jblok where JBlok_JScreen_ID='+
      Src_rJScreen.JScrn_JScreen_UID+' and '+
      '((JBlok_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(JBlok_Deleted_b IS NULL))';
  ...
  begin
    JASPrintln(saQry);
    repeat
      JASPrintln('JAS_JScreenCopy - copy: '+Src_rJScreen.JScrn_JScreen_UID+' DBScreen: '+rs.fields.get_saValue('JBlok_JScreen_ID')+' JBlok: '+ rs.fields.get_saValue('JBlok_JBlok_UID'));
      clear_jblok(Src_rJBlok);
      clear_jblok(Dest_rJBlok);
      if bOk then
      begin
        Src_rJBlok.JBlok_JBlok_UID:=rs.fields.get_saValue('JBlok_JBlok_UID');
        bOk:=bJAS_Load_JBlok(p_Context, DBC, Src_rJBlok, false);
        if not bOk then
        begin
          JAS_LOG(p_Context,cnLog_Error,201009121759,'Screen Copy - Trouble loading screen section (JBlok).',
            'bJAS_Load_JBlok for Src_rJBlok.JBlok_JBlok_UID: '+Src_rJBlok.JBlok_JBlok_UID,SOURCEFILE);
        end;
      end;
      ...
      begin
        ...
        Dest_rJBlok.JBlok_Name                 :=Dest_rJScreen.JScrn_Name+' '+
          DBC.saGetValue(DBC.ID,'jbloktype','JBkTy_JBlokType_UID='+DBC.saDBMSUintScrub(Src_rJBlok.JBlok_JBlokType_ID)+' AND '+
            '((JBkTy_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(JBkTy_Deleted_b IS NULL))');
        iBlokCount:=DBC.u8GetRowCount('jblok','((JBlok_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(JBlok_Deleted_b IS NULL)) AND '+
                      '(JBlok_JScreen_ID='+DBC.saDBMSUIntScrub(Dest_rJScreen.JScrn_JScreen_UID)+')',201506171756);
        ...
        bOk:=bJAS_Save_JBlok(p_Context, DBC, Dest_rJBlok, false,false);
        if not bOk then
        begin
      ...
      if bOk then
      begin
        saQry:='select JBlkF_JBlokField_UID from jblokfield where JBlkF_JBlok_ID='+Src_rJBlok.JBlok_JBlok_UID+' '+
          'AND ((JBlkF_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(JBlkF_Deleted_b IS NULL))';
      ...
      begin
        ...
        repeat
          ...
          Src_rJBlokField.JBlkF_JBlokField_UID:=rs2.fields.get_saValue('JBlkF_JBlokField_UID');
          bOk :=bJAS_Load_JBlokField(p_Context, DBC, Src_rJBlokField,false);
          if not bOk then
          begin
            JAS_LOG(p_Context,cnLog_Error,201009121905,'Screen Copy - Trouble loading a screen field. (JBlokfield)',
              'bJAS_Load_JBlokField for Src_rJBlokField.JBlkF_JBlokField_UID: '+ Src_rJBlokField.JBlkF_JBlokField_UID,SOURCEFILE);
          end;
          ...
          begin
            ...
            bOk:=bJAS_Save_JBlokField(p_Context, DBC, Dest_rJBlokField, falsE,false);
            if not bOk then
            begin
      ...
      if bOk then
      begin
        saQry:='select JBlkB_JBlokButton_UID from jblokbutton where JBlkB_JBlok_ID='+Src_rJBlok.JBlok_JBlok_UID +' '+
          'AND ((JBlkB_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(JBlkB_Deleted_b IS NULL))';
      ...
      //  if not bOk then
      //  begin
      //    saErrCode:='201009121806';
      //    saErrMsg:='JAS_JScreenCopy - Expected records not present. Query: '+ saQry+' '+SOURCEFILE;
      //  end;
      //end;
      ...
      begin
        ...
        begin
          JASPrintln('JAS_JScreenCopy - copy jblokbutton records');
          repeat
            clear_jblokbutton(Src_rJBlokButton);
            clear_jblokbutton(Dest_rJBlokButton);
            Src_rJBlokButton.JBlkB_JBlokButton_UID:=rs2.fields.get_saValue('JBlkB_JBlokButton_UID');
            bOk :=bJAS_Load_JBlokButton(p_Context, DBC, Src_rJBlokButton,false);
            if not bOk then
            begin
              JAS_LOG(p_Context,cnLog_Error,201009121807,'Screen Copy - Trouble loading a button.',
                'bJAS_Load_JBlokButton - for Src_rJBlokButton.JBlkF_JBlokButton_UID: '+ Src_rJBlokButton.JBlkB_JBlokButton_UID,SOURCEFILE);
            end;
            ...
            begin
              ...
              bOk:=bJAS_Save_JBlokbutton(p_Context, DBC, Dest_rJBlokbutton, false,false);
              if not bOk then
              begin
  ...
  //begin
  //  Dest_rJScreen.JScrn_JScreen_UID:='0';
  //  bOk:=bJAS_Save_JScreen(p_Context, Dest_rJScreen, false,false);
  //  if not bOk then
  //  begin
  //    JAS_LOG(p_Context,cnLog_Error,201009121625,'Screen Copy - Unable to create new Data Screen','',SOURCEFILE);
  //  end;
  //end;
  ...
  rs.destroy;
  rs2.destroy;
  JASPrintln('JAS_JScreenCopy - END');
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201210291312, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  if bOk then
  begin
    bOk:=p_Context.rJUser.JUser_Admin_b and (garJVHostLight[p_Context.i4VHost].saServerDomain='default');
    if not bOk then
    begin
      JAS_LOG(p_Context, cnLog_Error, 201210291314, 'Access Denied','User Admin: '+
        satrueFalsE(p_Context.rJUser.JUser_Admin_b)+' VHost ServerDomain: '+garJVHostLight[p_Context.i4VHost].saServerDomain,SOURCEFILE);
    end;
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201210291330, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    //    ' Name: '+saName+' LeadJet: '+saTrueFalse(bLeadJet),'',SOURCEFILE);
    //end;
  end;
  p_Context.XML.AppendItem_saName_N_saValue('result',lowercase( trim(saTrueFalse( bok))));
  p_Context.XML.AppendItem_saName_N_saValue('JDCon_JDConnection_UID',inttostr(JDCon_JDConnection_UID));
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201211061924, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  JASPrintln('Create Virtual Host Started -------------------');
  ...
  if bOk then
  begin
    saName:=saDecodeURI(lowercase(p_Context.CGIENV.DataIn.Get_saValue('name')));
    saServerName:=saDecodeURI(p_Context.CGIENV.DataIn.Get_saValue('servername'));
    JAS_LOG(p_Context, cnLog_Debug,201211241201,'ServerName from CGIENV: '+saServerName,'',SOURCEFILE);
    
    saFromEmailAddress:=saDecodeURI(p_Context.CGIENV.DataIn.Get_saValue('fromemail'));
 
    saWhere:='((VHost_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(VHost_Deleted_b IS NULL)) AND '+
             '((VHost_ServerIdent='+DBC.saDBMSScrub(saName)+')OR('+
             'VHost_ServerDomain='+DBC.saDBMSScrub(saName)+'))';
    bOk:=DBC.u8GetRowCount('jvhost',saWhere,201506171757)=0;
    if not bOk then
    begin
      JAS_LOG(p_Context, cnLog_Error,201211091352,'New JET Name exists in Virtual Host Table. Name: '+saName,'',SOURCEFILE);
    end;
  end;

  if bOk then
  begin
    saWhere:='((Alias_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(Alias_Deleted_b IS NULL)) AND '+
      '(Alias_Alias='+DBC.saDBMSScrub(saName)+')';
    bOk:=DBC.u8GetRowCount('jalias',saWhere,201506171758)=0;
    if not bOk then
    begin
  ...
  if bOk then
  begin
    saWhere:='((JDCon_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(JDCon_Deleted_b IS NULL)) AND '+
      '(JDCon_Name='+DBC.saDBMSScrub(saName)+')';
    bOk:=DBC.u8GetRowCount('jdconnection',saWhere,201506171759)=0;
    if not bOk then
    begin
      JAS_LOG(p_Context, cnLog_Error,201211091353,'New JET Name exists in Connection Table. Name: '+saName,'',SOURCEFILE);
    end;
  end;

  if bOk then
  begin
    saDestPath:=grJASCOnfig.saSysDir+'jets'+csDOSSLASH+saName;
  ...
  begin
    ...
    rJDCon.JDCon_JDConnection_UID:=inttostr(JDCon_JDConnection_UID);
    bOk:=bJAS_Load_JDConnection(p_Context, DBC, rJDCon, true);
    if not bOk then
    begin
  ...
  if bOk then
  begin
    saLogDir:='..'+csDOSSLASH+'jets'+csDOSSLASH+saName+csDOSSLASH+'log'+csDOSSLASH;
    clear_JVHost(rJVHost);
    with rJVHost do begin
      VHost_JVHost_UID               :='0';
      VHost_WebRootDir               :='..'+csDOSSLASH+'jets'+csDOSSLASH+saName+csDOSSLASH+'webroot'+csDOSSLASH;
      //VHost_WebRootDir               :='..'+csDOSSLASH+'webroot'+csDOSSLASH;
      VHost_ServerName               :=saServerName;
      VHost_ServerIdent              :=saName;
      VHost_ServerDomain             :=saName;
      VHost_ServerIP                 :=grJASConfig.saServerIP;
      VHost_ServerPort               :=inttostr(grJASConfig.u2ServerPort);
      VHost_DefaultLanguage          :='en';
      VHost_DefaultColorTheme        :='default';
      ...
      VHost_DefaultIconTheme         :='CrystalClear';
      VHost_DirectoryListing_b       :='true';
      VHost_FileDir                  :='..'+csDOSSLASH+'jets'+csDOSSLASH+saName+csDOSSLASH+'file'+csDOSSLASH;
      VHost_AccessLog                :=saLogdir+'access.'+saName+'.log';
      VHost_ErrorLog                 :=saLogDir+'error.'+saName+'.log';
      VHost_JDConnection_ID          :=inttostr(JDCon_JDConnection_UID);
      VHost_Enabled_b                :='true';
      VHost_CacheDir                 :='..'+csDOSSLASH+'jets'+csDOSSLASH+saName+csDOSSLASH+'cache'+csDOSSLASH;
      VHost_TemplateDir              :='..'+csDOSSLASH+'jets'+csDOSSLASH+saName+csDOSSLASH+'templates'+csDOSSLASH;
    end;//with
    bOk:=bJAS_Save_JVHost(p_COntext, DBC, rJVHost, false,false);
    if not bOk then
    begin
  ...
  begin
    ...
    bOk:=bJAS_Save_JIndexFile(p_Context, DBC, rJIndexFile1, false,false);
    if not bOk then
    begin
  ...
  begin
    ...
    bOk:=bJAS_Save_JIndexFile(p_Context, DBC, rJIndexFile2, false,false);
    if not bOk then
    begin
  ...
  begin
    ...
    bOk:=bJAS_Save_JPerson(p_Context, TGT, rJPerson, false, false);
    if not bOk then
    begin
  ...
  begin
    ...
    with rJUser do begin
      ...
      JUser_Audit_b                :=false;
      JUser_ResetPass_u            :=u8Val(saJAS_GetNextUID);
      JUser_JVHost_ID              :=rJVhost.VHost_JVHost_UID;
      JAS_Row_b                    :=true;
    end;//with
    bOk:=bJAS_Save_JUser(p_Context, TGT, rJUser, false, false);
    if not bOk then
    begin
  ...
  begin
    ...
    saQry:='UPDATE juser SET JUser_JUser_UID='+TGT.saDBMSUIntScrub(rJUser.JUser_JUser_UID);
  ...
  begin
    saQry:='update jmenu set JMenu_Name_EN=''Syncronization'', JMenu_Title_EN=''Synchronization'' where JMenu_JMenu_UID=1442';
  ...
  begin
    ...
    with rJSecGrpUserLink do begin
      ...
      JSGUL_JSecGrp_ID            :=inttostR(cnJSecGrp_Administrators);
      JSGUL_JUser_ID              :=inttostR(rJUser.JUser_JUser_UID);
    end;//with
    bOk:=bJAS_Save_JSecGrpUserLink(p_Context, TGT, rJSecGrpUserLink, false, false);
    if not bOk then
    begin
  ...
  begin
    ...
    with rJSecPerm do begin
      JSPrm_JSecPerm_UID        :='0';
      JSPrm_Name                :='DB Connection '+rJDCon.JDCon_Name;
    end;//with
    bOk:=bJAS_Save_JSecPerm(p_Context, DBC, rJSecPerm, false,false);
    if not bOk then
    begin
  ...
  if bOk then
  begin
    saQry:='INSERT INTO jsecperm (JSPrm_JSecPerm_UID) VALUES ('+DBC.saDBMSUIntScrub(rJSecPerm.JSPrm_JSecPerm_UID)+')';
  ...
  if bOk then
  begin
    bOk:=bJAS_Save_JSecPerm(p_Context, TGT, rJSecPerm, false,false);
    if not bOk then
    begin
  ...
  begin
    ...
    with rJSecGrpLink do begin
      ...
      JSGLk_JSecPerm_ID         :=rJSecPerm.JSPrm_JSecPerm_UID;
      JSGLk_JSecGrp_ID          :=inttostr(cnJSecGrp_Administrators);
    end;//with
    bOk:=bJAS_Save_JSecGrpLink(p_Context, TGT, rJSecGrpLink, false, false);
    if not bOk then
    begin
  ...
  if bOk then
  begin
    rJDCon.JDCon_JSecPerm_ID:=rJSecPerm.JSPrm_JSecPerm_UID;
    bOk:=bJAS_Save_JDConnection(p_Context, DBC, rJDCon, false, false);
    if not bOk then
    begin
  ...
  begin
    ...
    if bCGIBased then
    begin
      createdir(garJVHostLight[p_COntext.i4VHost].saWebRootDir+saName);
      saCGIFileSrc:='../bin/nph-jas.cgi';
      saCGIFileDest:=garJVHostLight[p_COntext.i4VHost].saWebRootDir+saName+csDOSSLASH+'nph-jas.cgi';
      ...
      begin
        ...
        begin
          ...
          JAS_LOG(p_Context, cnLog_Error, 201211120014, 'Unable to configure CGI program.','Result: '+inttostr(u2IOResult)+' '+saIOResult(u2IOResult),SOURCEFILE);
        end;
      end;
  ...
  if bOk then
  begin
    CreateDir(grJASConfig.saSysDir+csDOSSLASH+'jets');
    ...
    sa:=grJASConfig.saSysDir+'webroot'+csDOSSLASH+'index.jas '+
      rJVHost.VHost_WebRootDir+csDOSSLASH+'index.jas';
    u2IOResult:=u2Shell('../bin/cpdir.sh', sa);
    bOk:=u2ioresult=0;
    if not bOk then
    begin
      JAS_LOG(p_Context, cnLog_Error, 201503131350, 'Unable to copy index.jas file. '+sa,
  ...
  if bOk then 
  begin
    sa:=grJASConfig.saSysDir+csDOSSLASH+'templates'+csDOSSLASH+'* '+
       rJVHost.VHost_TemplateDir+'.';
    u2IOResult:=u2Shell('../bin/cpdir.sh', sa);
    bOk:=u2ioresult=0;
    if not bOk then
    begin
      JAS_LOG(p_COntext, cnLog_Error, 201503131357,'Unable to copy templates folder: ' +sa,'',SOURCEFILE);       
    end;
  end;
  ...
  begin  
    ...
    bOk:=bJAS_Save_JAlias(p_Context,DBC,rJAlias,false,false);
    if not bOk then
    begin
  ...
  if bOk then
  begin
    saQry:='UPDATE juser set JUser_JVHost_ID='+DBC.saDBMSUIntScrub(rJVHost.VHost_JVHost_UID)+' WHERE JUser_JUser_UID='+DBC.saDBMSUIntScrub(p_Context.rJUser.JUser_JUser_UID);
  ...
  begin
    ...
    bOk:=bEmail_ResetPassword(p_Context,'1', inttostr(rJUser.JUser_ResetPass_u), rJPerson.JPers_Work_Email,saWelcomeMsg, saName);
    if not bOk then
    begin
  ...
  begin
    if not bEmail_Generic(p_Context,
      'JAS - We''re Sorry - We were unable to create your JAS Jet',
      '<h2>We''re Sorry</h2>'+csCRLF+
      '<p>The System encountered an error while creating your CRM. We will address this as soon as possible for you!</p>'+csCRLF+
      '<p>Error code: '+inttostr(p_Context.u8Errno)+' '+p_Context.saErrMsg+'</p>'+csCRLF+
      'Regards,<br />'+csCRLF+
      '&nbsp;&nbsp;The Management'+csCRLF
    ) then
    begin
      JAS_Log(p_Context,cnLog_Error,201211231233,'Unable to send create jet failure email notice.','',SOURCEFILE);
    end;
  end;

  p_Context.XML.AppendItem_saName_N_saValue('result',lowercase( trim(saTrueFalse( bok))));
  JASPrintln('Create Virtual Host Finished. Result: '+saTrueFalse(bOk)); 
  TGT.CloseCon;
  TGT.Destroy;
  rs.destroy;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201211240015, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    if bOk then
    begin
      saQry:='SELECT JUser_JVHost_ID from juser WHERE ((JUser_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(JUser_Deleted_b IS NULL)) AND '+
        '(JUser_JUser_UID='+DBC.saDBMSUIntScrub(p_Context.rJUser.JUser_JUser_UID)+')';
    ...
    begin
      saQry:='SELECT VHost_SharesDefaultDomain_b, VHost_ServerDomain, VHost_EnableSSL_b FROM jvhost WHERE '+
        '((VHost_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(VHost_Deleted_b IS NULL)) AND '+
        '(VHost_JVHost_UID='+DBC.saDBMSUintScrub(saHostID)+')';
    ...
    begin
      ...
      rs.close;
      saQry:='SELECT Alias_Alias FROM jalias WHERE (Alias_JVHost_ID = '+DBC.saDBMSUIntScrub(saHostID)+') AND '+
        '((Alias_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')OR(Alias_Deleted_b IS NULL))';
    ...
    begin
      ...
      begin
        ...
        begin
          saAlias:=grJASConfig.saServerURL+'/'+saAlias+'/';
        end
        else
        begin
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201212090712, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      if garJVHostLight[i].u8JDConnection_ID<>0 then
      begin
        //ASPrintln('UpdateJetUSerAvg - Non JAS VHost DB Con Found');
        if JADO.bFoundConnectionByID(garJVHostLight[i].u8JDConnection_ID, pointer(p_Context.JADOC), iDBX) then
        begin
          ...
          u8ActiveUserCount:=TGT.u8GetRowCount(
            'juser','((JUser_Deleted_b<>'+TGT.saDBMSBoolScrub(true)+')OR(JUser_Deleted_b IS NULL)) AND '+
            'JUser_Enabled_b='+TGT.saDBMSBoolScrub(true),201506171760);
          ...
          clear_JUser(rJUser);
          rJUser.JUser_JUser_UID:=u8Val(DBC.saGetValue('juser','JUser_JUser_UID','JUser_JVHost_ID='+DBC.saDBMSUIntScrub(garJVHostLight[i].u8JVHost_UID)));
          ...
          begin
            ...
            if not bOk then
            begin
              JAS_LOG(p_Context,cnLog_Error,201212100720,'BILLING: Unable to load JPerson: '+rJPerson.JPers_JPerson_UID,'', SOURCEFILE);
            end;
          end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                saMessage:=
                  ...
                  'your billing information so that we will be able to '+
                  'continue using your Jegas CRM uninterrupted.'+csCRLF+csCRLF+
                ...
                if not bJAS_Sendmail(p_Context, garJVHostLight[0].sSystemEmailFromAddress, rJPerson.JPers_Work_Email, saSubject, saMessage) then
                begin
                ...
                if not bJAS_Sendmail(p_Context, garJVHostLight[0].sSystemEmailFromAddress, garJVHostLight[0].sSystemEmailFromAddress, saSubject, saMessage) then
                begin
            ...
            begin
              ...
              fdTotalJetUsers:=fdVal(DBC.saGetValue('juser','JUser_TotalJetUsers_d','JUser_JUser_UID='+DBC.saDBMSUIntScrub(rJUSer.JUser_JUser_UID)));
              ...
              if bOk then
              begin
                saQry:='UPDATE juser SET JUser_TotalJetUsers_d='+DBC.saDBMSUIntScrub(0)+
                  ' WHERE JUser_JUser_UID='+DBC.saDBMSUIntScrub(rJUser.JUser_JUser_UID);
            ...
            begin
              ...
              saQry:='UPDATE juser SET JUser_TotalJetUsers_d=JUser_TotalJetUsers_d + '+
                DBC.saDBMSUIntScrub(u8ActiveUserCount)+' WHERE ((JUser_Deleted_b<>'+
                DBC.saDBMSBoolScrub(true)+')OR(JUser_Deleted_b IS NULL)) AND '+
                'JUser_Enabled_b='+DBC.saDBMSBoolScrub(true)+' AND JUser_JUser_UID='+
                DBC.saDBMSUintScrub(rJUSer.JUser_JUser_UID);
              ...
              rs.close;
              //ASPrintln('UpdateJetUSerAvg - bOk: '+saTruefalse(bOk)+' Qry: '+saQry);
            end;
          end;
        end
        else
        begin
          bOk:=false;
          JAS_LOG(p_Context,cnLog_Error,201212100614,'BILLING: Unable to find DB Connection for Jet '+garJVHostLight[i].saServerName,'Query: '+saQry, SOURCEFILE, DBC, rs);
        end;
      end;
  ...
  p_Context.XML.AppendItem_saName_N_saValue('result',saTrueFalse(bOk));
  rs.destroy;
{$IFDEF DEBUGLOGBEGINEND}
  DebugOUT(sTHIS_ROUTINE_NAME,SourceFile);
{$ENDIF}
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201507120001, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    saValue:=DBC.saGetValue(saTable,saField,saWhere);
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201507130001, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin                                         
    ...
    saHelpID:=inttostr(u8Val(DBC.saGetValue('juserpref','UserP_Help_ID',
      '(UserP_UserPref_UID='+saUID+ ') and (UserP_Deleted_b<>true)')));
    bOk:=saHelpID<>'0';
    if not bOk then
    begin
      //p_Context.bErrorCondition:=true;
  ...
  begin
    saQry:=
      'SELECT '+
        'Help_Name_'+p_Context.saLang+' ,'+
        'Help_HTML_'+p_Context.saLang;
    if p_Context.rJUser.JUser_Admin_b then
    begin
      saQry+=
        ' , Help_HTML_Adv_'+p_COntext.saLang+' ';
    end;
    saQry+=
      'FROM jhelp '+
  ...
  begin
    if not rs.eol then
    begin
      saResult:=rs.fields.Get_saValue('Help_Name_'+p_COntext.saLang)+' '+
                rs.fields.Get_saValue('Help_HTML_'+p_COntext.saLang);
      if p_Context.rJUser.JUser_Admin_b then
      begin
        saResult+=' '+rs.fields.Get_saValue('Help_HTML_Adv_'+p_COntext.saLang);
      end;
    end;
  end;
