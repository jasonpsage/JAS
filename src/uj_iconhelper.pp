  ...
  {$IFDEF ROUTINENAMES}sTHIS_ROUTINE_NAME: String;{$ENDIF}
                            ...
                            begin
                              ...
                              begin
                                ...
                                repeat
                                  //ASPrintLn('test 170');
                                  result+=TK.Item_saToken+DOSSLASH;
                            ...
                            begin
                              ...
                              begin
                                saSP:=saPath+inttostR(p_iSize)+DOSSLASH;
                              end;
                              ...
                              begin
                                ...
                                begin
                                  JAS_Log(p_Context,cnLog_Error, 201008020452, 'Icon Helper - Trouble copying Icon place holder file. IO Result:'+
                                   sIOResult(u2IOResulT) ,'Source: '+saSrcIconHelper+' Destination: '+saDestfile,SOURCEFILE,nil,nil);
                                  RenderHtmlErrorPage(p_Context);
                                end
                                else
                                begin
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102341, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  if bOk then
  begin
    bOk:=bJAS_HasPermission(p_Context, cnPerm_Icon_Helper);
    If not bOk Then
    Begin
  ...
  begin
    ...
    begin
      if saRightStr(saPathDefaultOveride,1)<>DOSSLASH then saPathDefaultOveride+=DOSSLASH;
      saPath:=saPathDefaultOveride;
    end
    else
    begin
      saPath:=grJASConfig.saSysDir+'webshare'+DOSSLASH+'img'+DOSSLASH+'icon'+DOSSLASH+'themes'+DOSSLASH+saTemplateName+DOSSLASH;
    end;
  ...
  if bOk then 
  begin
    saSrcMissingIconFile:=grJASConfig.saSysDir+'webshare'+DOSSLASH+'img'+DOSSLASH+'icon'+DOSSLASH+'JAS'+DOSSLASH+'iconhelper'+DOSSLASH+'missing-[@SIZE@].png';
  end;    
  
  //ASPrintLn('test 70');
  if bOk then
  begin
    saSP:=saPath+'fixed'+DOSSLASH;
  ...
  if bOk then
  begin
    saQry:='select * from jiconcontext where IHCO_Deleted_b<>'+DBC.sDBMSBoolScrub(true);
    ...
    repeat
      ...
      begin
        ...
        begin
          ...
          begin
            case iSize of 
             1: begin saSP:=saPath+'16'+DOSSLASH;   end;
             2: begin saSP:=saPath+'22'+DOSSLASH;   end;
             3: begin saSP:=saPath+'24'+DOSSLASH;   end;
             4: begin saSP:=saPath+'32'+DOSSLASH;   end;
             5: begin saSP:=saPath+'48'+DOSSLASH;   end;
             6: begin saSP:=saPath+'64'+DOSSLASH;   end;
             7: begin saSP:=saPath+'72'+DOSSLASH;   end;
             8: begin saSP:=saPath+'96'+DOSSLASH;   end;
             9: begin saSP:=saPath+'128'+DOSSLASH;  end;
            10: begin saSP:=saPath+'256'+DOSSLASH;  end;
            11: begin saSP:=saPath+'512'+DOSSLASH;  end;
            end;//switch
      ...
      begin
        ...
        saQry:='select * from jiconmaster '+
          ...
          'IHMA_Deleted_b<>'+DBC.sDBMSBoolScrub(true);
...
//): AnsiString;
  ...
  if bOk then
  begin
    p_Context.saPage:=saGetPage(p_Context,'','sys_page_iconhelper','main',false,'',123020100052);
  end;
{$IFDEF DEBUGLOGBEGINEND}
  DebugOut(sTHIS_ROUTINE_NAME,SourceFile);
{$ENDIF}
