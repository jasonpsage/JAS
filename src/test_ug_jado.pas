...
//=============================================================================
{
Function bTest_bLoadConnectionFromFile: Boolean;
Begin
  saFilename:='Test_ODBC.dsn';
  //saFileName:='Test_MySQL04.dsn';
  Result:=bLoadConnectionFromFile(saFilename,rcon);
End;


Function bTest_bSaveConnectiontoFile: Boolean;
Begin
  saFilename:='Test_ODBCOutput.dsn';
  Result:=bSaveConnectionToFile(saFilename,rcon);
End;
...
Begin
  ...
  //oADOC:=JADO_CONNECTION.Create(cnDriv_ODBC);
  oADOC:=JADO_CONNECTION.Create(rCon.i4DrivID);
  //cnDriv_MySQL);
  ...
  Writeln('oADOC.i4State = ',oADOC.i4State);
  
  
  Write('oADOC.i4DrivID = ');
  Case oADOC.i4DrivID Of
  cnDriv_ODBC: Writeln('JADO ODBC Driver');
  cnDriv_MySQL: Writeln('JADO MySQL Driver');
  End;//case
  
  Result:=oADOC.i4State = adStateOpen;
End;
}
...
Begin
  ...
  Writeln(';/_____/   /______/ /______/ /_/ /_/ /______/   |    Copyright(C)2006 Jegas    ');
...
Begin
  For i:=1 To 999 Do Begin
    sa:='jadoclu_session.'+saZeroPadInt(i,3)+'.txt';
    If FileExists(sa) Then DeleteFile(sa);
  End;
End;
...
Begin
  ...
  Begin
    ...
    Begin Writeln(saIOResult(IOR));End;
    {$I+}
    ...
    Begin
      repeat
        ...
        {$I+}
        IOR:=IOResult;If IOR<>0 Then
        Begin close(fout); Writeln(saIOResult(IOR));End;
...
Begin
  ...
  else write('Unknown Value:' + saStr(JADOC.u2DrivID));
  End;//case
  Write(' ');
  Writeln;

  Write('Name:',JADOC.saName,' ');
  Writeln;
  
  Write('Desc:',JADOC.saDesc,' ');
  Writeln;
  
  Write('DSN:',JADOC.saDSN,' ');
  Writeln;
  
  Write('U:',JADOC.saUserName,' ');
  Writeln;
  Write('P:',JADOC.saPassword,' ');
  Writeln;
  Write('Driver:',JADOC.saDriver,' ');
  Writeln;
  Write('Server:',JADOC.saServer,' ');
  Writeln;
  Write('DataBase:',JADOC.saDatabase,' ');
  Writeln;
  ...
  Else
  Begin
    Case JADOC.i4State Of
    adStateClosed     : Write('CLOSED');
    adStateConnecting : Write('CONNECTING'); 
    adStateExecuting  : Write('EEXECUTING');
    adStateFetching   : Write('FETCHING');
    adStateOpen       : Write('OPEN');
    End;//case
  End;
  Write(' ');
  Writeln;
  ...
  End;
  Writeln;
  Writeln('Verbose:',saOnOff(bVerbose));
  Writeln;
  Writeln('///////////////////////////////////////////////////////////////////////////////');
  Writeln;
...
Begin
  ...
  repeat
    ...
    Begin
      ...
      Writeln('?=Help Q=Quit 1=Test_ODBC.dsn 2=Test_MySQL.dsn');
      Writeln('You may also just type a filename.');
      saFile:='';
      bHandled:=True;
    End;
...
Begin
  ...
  Else
  Begin
    Writeln('Error:',saIOResult(u2IOResult));
  End;
  Writeln('----------------------------------------');
  Writeln;
End;
    ...
    Begin
      ...
      Begin
        ...
        Begin
          ...
          Writeln('Verbose is now ' + saOnOff(bVerbose));
        End;
      End;          
      If (not bHandled) Then
      Begin
        ...
        Begin
          bHandled:=True;
          If bFileExists(csLOG_FILENAME_DEFAULT) Then
          Begin
          ...
          Begin
            ...
            Begin
              ...
              Begin
                ...
                If JADOR.Open(saCommand,JADOC) Then
                Begin
                  ...
                  Begin
                    ...
                    begin
                      JADOC.Errors.MoveFirst;
                      repeat
                        writeln('ERR '+ saStr(JADOC.errors.n) +':'+ inttostr(JADO_ERROR(JADOC.errors.lpitem).i8Number) +' '+ JADO_ERROR(JADOC.errors.lpitem).saSource);
                      until not JADOC.Errors.MoveNext;
                    end;
                  End
                  Else
