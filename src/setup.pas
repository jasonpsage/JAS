...
begin
  ...
  saFilenameJASConfig:=saJASInstallDir+'config'+DOSSLASH+'jas.cfg';

  bWriteFile:=FileExists(saFilenameJASConfig)=false;
  ...
  saFilenameJASDSN:=saJASInstallDir+'config'+DOSSLASH+'database'+DOSSLASH+'jas.dsn';
  bWriteFile:=FileExists(saFilenameJASDSN)=false;
  if not bWritefile then
  begin
  ...
  begin
    {$IFDEF WIN32}
    saFilenamePopDB:=saJASINstallDir+'database'+DOSSLASH+'populate_database.bat';
    {$ELSE}
    saFilenamePopDB:=saJASINstallDir+'database'+DOSSLASH+'populate_database.sh';
    {$ENDIF}

    if FileExists(saFilenamePopDB) then
    begin
    ...
    writeln(f,'mysql -u' + saMySqlRootName + ' -p'+saMySqlRootPass  + ' < '+
      saJASInstallDir+'database'+DOSSLASH+'jas_stock.sql');
    close(f);
    {$IFNDEF WIN32}
    ...
    if iResult<>0 then
    begin
      msgbox(saFilenamePopDB+' Result:'+inttostr(iResult)+':'+sIOResult(iResult),'Trouble Loading Database',MB_OK);
    end
    else
    begin
...
begin
  ...
  begin
    ...
    repeat 
top: 
      gWinEx.Caption:=p_StepXXDL.sClassName;
      ...
      SS_SHELL: begin // execute external process
        ...
        begin
          ...
          begin
            saErrMsg+=' '+sIOResult(iResult-32768,cnLanguage);
          end;
        end;
      end;
      //----------------------
      ...
      SS_MAKEDIR: begin //create dir
        ...
        if not bOk then
        begin
          saErrMsg:='Error creating directory: '+saCmd+' Directory Exists:'+sTrueFalse(directoryexists(saCmd));
        end;
      end;
      //----------------------
      ...
      SS_COPYFILE: begin
        ...
        begin
          ...
          begin
            bOk:=false;
            saErrMsg:='Unable to Copy source file "'+saSrc+'" to destination "'+saDest+'" in step named "'+CurItem.saName+'" IO Result: '+sIOResult(u2IOResult);
          end
        end;
        p_StepXXDL.FoundNth(StepBookMarkN);
      end;
      ...
      SS_COPYFILESNR: begin
        ...
        if bOk then
        begin
          //saSrc:=saSpecialSNR(saSrc,DOSSLASH,DOSSLASH);
          //saDest:=saSpecialSNR(saDest,DOSSLASH,DOSSLASH);
          //MSGBOX('Load Test File','Source:'+saSrc+' Dest:'+saDest,MB_OK);
          bOk:=bLoadTextFile(saSrc,saFileData,u2IOResult);
          if not bOk then 
          begin
            saErrMsg:='Setup Script Failure: COPY-FILE-SNR Step named "'+CurItem.saName+'" was unable to load file: "'+saSrc+'" IO Result:'+sIOResult(u2IOResult);
          end;
        end;
        ...
        begin
          ...
          if not bOk then
          begin
            saErrMsg:='Setup Script Failure: COPY-FILE-SNR Step named "'+CurItem.saName+'" was unable to assign file handle for "'+saDest+'" IO Result:'+sIOResult(u2IOResult);
          end;
        end;
        ...
        begin
          ...
          if not bOk then
          begin
            saErrMsg:='Setup Script Failure: COPY-FILE-SNR Step named "'+CurItem.saName+'" was unable to open for write file named "'+saDest+'" IO Result:'+sIOResult(u2IOResult);
          end;
        end;            
        ...
        begin
          ...
          if not bOk then
          begin
            saErrMsg:='Setup Script Failure: COPY-FILE-SNR Step named "'+CurItem.saName+'" was unable to write to file named "'+saDest+'" IO Result:'+sIOResult(u2IOResult);
          end;
        end;            
        ...
        begin
          ...
          if not bOk then
          begin
            saErrMsg:='Setup Script Failure: COPY-FILE-SNR Step named "'+CurItem.saName+'" was unable to close this file:"'+saDest+'" IO Result:'+sIOResult(u2IOResult);
          end;
        end;            
        p_StepXXDL.FoundNth(StepBookMarkN);
      end;
...
begin
  ...
  MSG_BUTTONDOWN: begin
    ...
    begin
      Con.CloseCon;
      Con.u8DbmsID:=cnDBMS_MySQL;
      Con.u8DriverID:=cnDriv_MySQL;
      Con.sUserName:=gWinDBMS_inpUser.Data;
      Con.sPassword:=gWinDBMS_inpPassword.Data;
      Con.sServer:=gWinDBMS_inpHost.Data;
      Con.sDatabase:=gWinDBMS_inpDatabase.Data;
      if Con.OpenCon then
      begin
...
begin
  with JASSetupXXDL do begin
    sClassName:='JAS - Jegas Application Server by Jegas, LLC - www.jegas.com - Setup';// name of Setup Process
    // Install?
...
begin
  with grJegasCommon do begin
    sAppTitle:='JAS Setup Utility';//<application title
    sAppProductName:='JAS';
    sVersion:='2016-10-04';
  end;//with
  with grJegasCommon do begin
    JegasLogoSplash(
      sAppTitle,
      sAppProductName+' Version: '+sVersion,
      'By: Jason P Sage',
