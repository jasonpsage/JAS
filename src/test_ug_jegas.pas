...
begin
  ...
  begin
    {$IFDEF WIN32}
    iShellResult:=ishell('cmd','/c time /t');
    bOk:=iShellResult=0;
    {$ELSE}
    iShellResult:=ishell('/usr/bin/time','-apvV');
    bOk:=iShellResult=0;
    {$ENDIF}
    if not bOk then
    begin
      write('iShell - result:',iShellResult);
      if(iShellResult>32768)then
      begin
        write(' This is a DosError Code - you need to subtract 32768 to get the real Error code, its: ', iShellResult-32768, ' which means error: ',saIOResult(word(iShellResult-32768)));
      end
      else
      begin
  ...
  begin
    {$IFDEF WIN32}
    iShellResult:=ishell('cmd','/c echo hello > test_uxxg_jegas_output.txt');
    bOk:=iShellResult=0;
    {$ELSE}
    assign(f,'test_uxxg_jegas_output.txt');
    rewrite(f);
    write(f,'hello');
    close(f);
    //iShellResult:=ishell('echo','hello > test_uxxg_jegas_output.txt');
    {$ENDIF}
    if not bOk then write('problem shelling out to make output file for testing purposes. '+saIOREsult(iShellresult));
    if bOk then
    begin
      bOk:=bFileExists('test_uxxg_jegas_output.txt');
      if not bOk then write('tried to make a test drop file, it doesn''t exist like it should.');
    end;
    
    if bOk then
    begin
      saSource:='test_uxxg_jegas_output.txt';
      saDest:='test_uxxg_jegas_output2.txt';
      bOk:=bCopyfile(saSource, saDest);
      if not bOk then 
      begin
        write('bCopyFile(src,dest) 201001270217');
        bOk:=bCopyfile(saSource, saDest,u2IOResult);
        writeln('bOk: '+saTrueFalse(bOk)+' ioresult:'+saIOREsult(u2IOResult));
      end;
      
      if bOk then 
      begin
        bOk:=bFileExists(saDest);
        if not bOk then write('bCopyFile(src,dest) file not exist afterwards.');
      end;
    end;
  end;
  ...
  begin
    ...
    if bOk then
    begin
      bOk:=bFileExists(saSource)=false;
  ...
  if bOk then 
  begin
    deletefile('test_uxxg_jegas_output.001.txt');
    deletefile('test_uxxg_jegas_output.002.txt');
    bOk:=(false=bFileExists('test_uxxg_jegas_output.001.txt')) and (false=bFileExists('test_uxxg_jegas_output.002.txt'));
    if not bOk then write('unable to remove test output files prior to test - they still exist and should not.');

    if bOk then
    begin
      {$IFDEF WIN32}
      bOk:=(0=ishell('cmd','/c echo hello > test_uxxg_jegas_output.001.txt'));
      {$ELSE}
      //bOk:=(0=ishell('echo hello > test_uxxg_jegas_output.001.txt',''));
      assign(f,'test_uxxg_jegas_output.001.txt');
      rewrite(f);
      write(f,'hello');
      close(f);
      {$ENDIF}
    ...
    if bOk then 
    begin
      bOk:=bFileExists('test_uxxg_jegas_output.001.txt');
      if not bOk then write('output file created for test doesn''t exist so unable to test.');
    end;
    
    if bOk then
    begin
      bOk:=(saSequencedFilename('test_uxxg_jegas_output','txt')='test_uxxg_jegas_output.002.txt');
      if not bOk then write('saSequencedFilename(filename,ext)');
    end;
    ...
    begin
      // Clean up :)
      deletefile('test_uxxg_jegas_output.001.txt');
      deletefile('test_uxxg_jegas_output.txt');
    end;
  end;
  ...
  begin
    {$IFDEF WIN32}
      ...
      bOk:=(saFileNameNoExt(saSource)='c:\somedir\somefile');
    {$ELSE}
      saSource:='/home/somefile.txt';
      bOk:=(saFileNameNoExt(saSource)='/home/somefile');
    {$ENDIF}
    if not bOk then write('saFileNameNoExt(src)');
  end;
...
begin
  ...
  begin
    ...
    if not bOk then write('tsAddHours failed:'+saFormatDateTime(csDATETIMEFORMAT,TimeStampToDateTime(tsAddHours(DateTimeToTimeStamp(dt), 24))));
  end;
  ...
  begin
    ...
    if not bOk then write('tsAddMinutes failed:'+saFormatDateTime(csDATETIMEFORMAT,TimeStampToDateTime(tsAddMinutes(DateTimeToTimeStamp(dt), 24))));
  end;
  ...
  begin
    ...
    if not bOk then write('tsAddSec failed:'+saFormatDateTime(csDATETIMEFORMAT,TimeStampToDateTime(tsAddSec(DateTimeToTimeStamp(dt), 24))));
  end;
  ...
  begin
    ...
    if not bOk then write('tsAddMSec failed:'+saFormatDateTime(csDATETIMEFORMAT,TimeStampToDateTime(tsAddMSec(DateTimeToTimeStamp(dt), 24))));
  end;
  ...
  //--------------------
  // SEE NOTES in uxxg_jegas.pp for this function - the results aren't 100% 
  ...
  begin
    ...
    writeln('RESULT:'+saFormatDateTime(csDATETIMEFORMAT,TimeStampToDateTime(tsDiff(DateTimeToTimeStamp(dt),ts))));
  ...
  begin
    ...
    bOk:='2005-01-01 17:30:00'=saFormatDateTime(csDATETIMEFORMAT,TimeStampToDateTime(ts));
    if not bOk then write('StrToTimeStamp');
  end;

  //=============================================================================
  // Function saFormatDateTime(p_saFormat: AnsiString; p_ts: TTIMESTAMP): AnsiString;
  //=============================================================================
  if bOk then
  begin
    Try
    ...
    Except
    End;
    bOk:='2005-01-01 5:30:00'=saFormatDateTime(csDATETIMEFORMAT,dt);
    if not bOk then write('saFormatDateTime');
  end;
  ...
  begin
    ...
    if not bOk then write('dtAddMonths failed:'+saFormatDateTime(csDATETIMEFORMAT,dtAddMonths(dt, 4)));
  end;
  ...
  begin
    ...
    if not bOk then write('dtAddDays failed:'+saFormatDateTime(csDATETIMEFORMAT,dtAddDays(dt2, 26)));
  end;
  ...
  begin
    ...
    if not bOk then write('dtAddHours failed:'+saFormatDateTime(csDATETIMEFORMAT,dtAddHours(dt2, 24)));
  end;
  ...
  begin
    ...
    if not bOk then write('dtAddMinutes failed:'+saFormatDateTime(csDATETIMEFORMAT,dtAddMinutes(dt2, 60)));
  end;
  ...
  begin
    ...
    if not bOk then write('dtAddSec failed:'+saFormatDateTime(csDATETIMEFORMAT,dtAddSec(dt2, 3600)));
  end;
  ...
  begin
    ...
    if not bOk then write('dtAddMSec failed:'+saFormatDateTime(csDATETIMEFORMAT,dtAddMSec(dt, 1000)));
  end;
