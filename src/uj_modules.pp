...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102449, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  saQry:='select JModu_JModule_UID from jmodule '+
    'where JModu_Name='+JAS.saDBMSScrub(p_JModu_Name)+' and '+
    'JModule.JModu_Deleted_b<>'+JAS.saDBMSBoolScrub(true);
  p_JModu_JModule_UID:='0';
  bOk:=rs.Open(saQry, DBC);
  if not bOk then
  begin
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102452, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  saQry:='select JModu_Name from jmodule '+
    ...
    '(JModule.JModu_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+')';
  p_JModu_Name:='';
  bOk:=rs.Open(saQry, DBC);
  if not bOk then
  begin
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102455, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  saQry:='select '+
         ...
         ' and (JModule.JModu_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+') and '+
         '(jinstalled.JInst_Deleted_b<>'+DBC.saDBMSBoolScrub(true)+') ';
  ...
  if not bOk then
  begin
    JAS_Log(p_Context,cnLog__Error,201006141229,'Trouble querying jmodule and jinstalled tables.','Query: '+saQry,SOURCEFILE,DBC,rs);
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102461, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  saQry:=
    ...
    'JMSet_Deleted_b<>'+DBC.saDBMSBoolScrub(true);
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102464, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  saQry:=
    'select  jmodule.*, jmodulesetting.*  from jmodule join jmodulesetting on JModu_JModule_UID=JMSet_JModule_ID '+
    'where jmodule.JModu_JModule_UID='+JAS.saDBMSScrub(p_JModu_JModule_UID)+' and '+
    'JMSet_Deleted_b<>'+JAS.saDBMSBoolScrub(true);
  ...
  if not bOk then
  begin
    JAS_Log(p_Context, cnLog_Error,201006141232,'Trouble querying jmodule and jmodulesetting tables.','Query: '+saQry,SOURCEFILE,JAS,rs);
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102467, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    saQry:='select JInst_JModule_ID from jinstalled '+
      ...
      'JInst_Deleted_b<>'+DBC.saDBMSBoolScrub(true);
    bOk:=rs.Open(saQry, DBC);
    if not bOk then
    begin
  ...
  begin
    if rs.eol=false then
    begin
      rJModule.JModu_JModule_UID:=rs.fields.Get_saValue('JModu_JModule_UID');
    end;
  end;
  rs.close;
  ...
  if bOk then
  begin
    bOk:=bGet_Setting_List_For_Given_Module_ID(p_Context, rJModule.JModu_JModule_UID, XDL);
    if not bOk then
    begin
      JAS_Log(p_Context, cnLog_Error,201006160137,'Trouble bGet_Setting_List_For_Given_Module_ID.','rJModule.JModu_JModule_UID: '+rJModule.JModu_JModule_UID,SOURCEFILE);
    end;
  end;
  ...
  begin
    ...
    if not bOk then
    begin
      JAS_Log(p_ContecnLog_TY_Error,201006160138,'XDL is nil and it should be after call to bGet_Setting_List_For_Given_Module_ID.','',SOURCEFILE);
    end;
  end;
    ...
    begin
      ...
      repeat
        saQry:='select * from jmoduleconfig '+
          ...
          ' ((JMCfg_JUser_ID is null) or (JMCfg_JUser_ID='+p_Context.rJSession.JSess_JUser_ID+')) and '+
          ' JMCfg_Deleted_b<>'+DBC.saDBMSBoolScrub(true);
        bOk:=rs.Open(saQry, DBC);
        if not bOk then
        begin
          JAS_Log(p_ContecnLog_TY_Error,201006160139,'Trouble querying jmoduleconfig table.','Query:'+saQry,SOURCEFILE,DBC,rs);
        end;
        
        if rs.eol=false then
        begin
          u8PermissionID:=u8Val(rs.fields.Get_saValue('JMCfg_Read_JSecPerm_ID'));
          if (i4PermissionID=0) or (bHasPermission(i4PermissionID, p_Context)) then
          begin
            p_XDL.AppendItem;
            p_XDL.Item_saName:=XDL.Item_saName;
            p_XDL.Item_saValue:=rs.fields.Get_saValue('JMCfg_Value');
            if(bJASNote(p_Context, rs.fields.Get_saValue('JMCfg_JNote_ID'), saNotesResult,''))then
            begin
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102470, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    if not bOk then
    begin
      JAS_Log(p_ContecnLog_TY_Error,201006160140,'Trouble querying jinstalled table.','Query: '+saQry,SOURCEFILE,DBC,rs);
    end;
  end;
  ...
  if bOk then
  begin
    rJModule.JModu_JModule_UID:=rs.fields.Get_saValue('JInst_JModule_ID');
  end;
  rs.close;
  ...
  begin
    saQry:='select JMSet_JModuleSetting_UID from jmodulesetting '+
      ' where JMSet_JModule_ID='+DBC.saDBMSScrub(rJModule.JModu_JModule_UID)+' and '+
  ...
  if bOk then
  begin  
    rJModuleSetting.JMSet_JModuleSetting_UID:=rs.fields.Get_saValue('JMSet_JModuleSetting_UID');
  end;
  rs.close;    
  ...
  begin
    saQry:='select * from jmoduleconfig '+
      'where JMCfg_JInstalled_ID='+DBC.saDBMSScrub(p_JInst_JInstalled_UID)+' and '+
      ' JMCfg_JModuleSetting_ID='+DBC.saDBMSScrub(rJModuleSetting.JMSet_JModuleSetting_UID)+' and '+
      ' ((JMCfg_JUser_ID is null) or (JMCfg_JUser_ID='+DBC.saDBMSUIntScrub(p_Context.rJSession.JSess_JUser_ID)+') or (JMCfg_JUser_ID=0)) and '+
      ' JMCfg_Deleted_b<>'+JAS.saDBMSBoolScrub(true);
    bOk:=rs.Open(saQry, DBC);
    if not bOk then
    begin
  ...
  begin
    ...
    if not bOk then
    begin
      JAS_Log(p_ContecnLog_TY_Error,201006160143,'Zero records returned querying jmoduleconfig table.','Query: '+saQry,SOURCEFILE,DBC,rs);
    end;
  end;  
  ...
  begin
    ...
    begin
      JAS_Log(p_ContextcnLog__Info,201006160144,'Permission not granted trying to get a module instance config value.',
        'i4Permission: '+inttostr(i4PermissionID)+' bHasPermission:'+saTrueFalse(bHasPermission(i4PermissionID, p_Context)),SOURCEFILE); 
    end;
