...
//=============================================================================
{}
//Entertainment - This Unit has Some Little Animiations
Unit uc_animate;
//=============================================================================
...
//  end;
  ...
  Begin
    ...
    garStar[t].Y:=random(giConsoleHeight+21);
    If t <= cn_StarZ1 Then
    Begin
      garStar[t].Z:=1;
...
//  end;
  ...
  Begin
    ...
    Begin
      ...
      garStar[t].Y:=random(giConsoleHeight+20); // allow certain amount of rnd *'s
    End;
...
Begin
  ...
  Begin
    ...
    If (i4TimeDiff>cn_BlockDelay) OR (i4TimeDiff<0) Then
    Begin
      gi4BlockTime:=iMSec;
      ...
      repeat
        ...
        End;
        If VCGetx<cn_BlocksMaxLeft Then 
        Begin
          TVC(BlockVC.lpitem).iDir:=random(8);
          VCSetXY(VCGetX+10, VCGetY);
        End;
        If VCGetx>cn_BlocksMaxRight Then 
        Begin
          TVC(BlockVC.lpitem).iDir:=random(8);
          VCSetXY(VCGetX-10, VCGetY);
        End;
        If VCGetY<cn_BlocksMaxUp Then 
        Begin
          TVC(BlockVC.lpitem).iDir:=random(8);
          VCSetXY(VCGetX, VCGetY+10);
        End;
        If VCGetY>cn_BlocksMaxDown Then 
        Begin
          TVC(BlockVC.lpitem).idir:=random(8);
          VCSetXY(VCGetX, VCGetY-10);
        End;
...
//  LightGray=7
  ...
  Begin
    ...
    Begin
      ...
      rx:=  cn_FishMaxRight-cn_FishMaxLeft;
      ry:=  cn_FishMaxDown-cn_FishMaxUp;
      
      tvc(FishVC.lpitem).iVCUID:=NewVC(
        random(rx)+cn_FishMaxLEft,
        random(ry)+cn_FishMaxUp,
  ...
  Begin
    ...
    repeat
      ...
      Begin
        ...
        If VCGetx<cn_FishMaxLeft Then 
        Begin
          TVC(FishVC.lpitem).iDir:=random(8);
          bDirChange:=True;
          VCSetXY(cn_FishMaxLeft, VCGetY);
        End;
        If (VCGetx+vcWidth-1)>cn_FishMaxRight Then 
        Begin
          TVC(FishVC.lpitem).iDir:=random(8);
          bDirChange:=True;
          VCSetXY(cn_FishMaxRight-VCWidth+1, VCGetY);
        End;
        If VCGetY<cn_FishMaxUp Then 
        Begin
          TVC(FishVC.lpitem).iDir:=random(8);
          bDirChange:=True;
          VCSetXY(VCGetX, cn_FishMaxUp);
        End;
        If (VCGetY+VCHeight-1)>cn_FishMaxDown Then 
        Begin
          TVC(FishVC.lpitem).iDir:=random(8);
          bDirChange:=True;
          VCSetXY(VCGetX, cn_FishMaxDown-VCHeight+1);
        End;
