...
//{IFDEF DEBUGTHREADBEGINEND}p_Context.JThread.DBIN(201203102434,sTHIS_ROUTINE_NAME,SOURCEFILE);{ENDIF}
  ...
  begin
    ...
    saQry:='SELECT * '+
      ' FROM jmenu '+
      ' WHERE ((JMenu_Deleted_b<>'+p_DBC.sDBMSBoolScrub(true)+')OR(JMenu_Deleted_b IS NULL)) '+
  ...
  Begin
    ...
    Begin
      ...
      repeat
        ...
        rJMenu^.JMenu_DisplayIfNoAccess_b        := false;
        rJMenu^.JMenu_Title_en                   := '';
        pointer(rJMenu^.JMenu_IconLarge)         := nil;
        pointer(rJMenu^.JMenu_IconSmall)         := nil;
        rJMenu^.JMenu_IconLarge_Theme_b          := false;
        rJMenu^.JMenu_IconSmall_Theme_b          := false;
        ...
        rJMenu^.JMenu_JSecPerm_ID                := 0;
        rJMenu^.JMenu_Name                    := '';
        rJMenu^.JMenu_NewWindow_b                := false;
        rJMenu^.JMenu_SEQ_u                      := 0;
        pointer(rJMenu^.JMenu_URL)               := nil;
        rJMenu^.JMenu_ValidSessionOnly_b         := false;
        rJMenu^.JMenu_DisplayIfValidSession_b    := false;
        rJMenu^.JMenu_ReadMore_b                 := false;

        //ASPrintln('C&L Menu Load Menu Record Structure with Data');
        rJMenu^.JMenu_DisplayIfNoAccess_b                 :=bVal(rs.Fields.Get_saValue('JMenu_DisplayIfNoAccess_b'));
        ...
        rJMenu^.JMenu_IconLarge_Theme_b                   :=bVal(rs.Fields.Get_saValue('JMenu_IconLarge_Theme_b'));
        rJMenu^.JMenu_IconSmall_Theme_b                   :=bVal(rs.Fields.Get_saValue('JMenu_IconSmall_Theme_b'));
        ...
        rJMenu^.JMenu_JSecPerm_ID                         :=u8Val(rs.Fields.Get_saValue('JMenu_JSecPerm_ID'));
        rJMenu^.JMenu_Name                                :=rs.Fields.Get_saValue('JMenu_Name');
        rJMenu^.JMenu_NewWindow_b                         :=bVal(rs.Fields.Get_saValue('JMenu_NewWindow_b'));
        rJMenu^.JMenu_SEQ_u                               :=uVal(rs.Fields.Get_saValue('JMenu_SEQ_u'));
        rJMenu^.JMenu_URL                                 :=saSNRStr(rs.Fields.Get_saValue('JMenu_URL'),'[@MID@]','MID='+inttostr(rJMenu^.JMenu_JMenu_UID));
        rJMenu^.JMenu_ValidSessionOnly_b                  :=bVal(rs.Fields.Get_saValue('JMenu_ValidSessionOnly_b'));
        rJMenu^.JMenu_DisplayIfValidSession_b             :=bVal(rs.Fields.Get_saValue('JMenu_DisplayIfValidSession_b'));
        rJMenu^.JMenu_ReadMore_b                          :=bVal(rs.Fields.Get_saValue('JMenu_ReadMore_b'));
...
//{IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102437, sTHIS_ROUTINE_NAME);{ENDIF}
  ...
  Begin
    ...
    Begin
      ...
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_DisplayIfNoAccess_b:=false;
      ...
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_IconLarge_Theme_b:=false;
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_IconSmall_Theme_b:=false;
      ...
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_JSecPerm_ID:=0;
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_Name:='';
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_NewWindow_b:=false;
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_SEQ_u:=0;
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_URL:='';
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_ValidSessionOnly_b:=false;
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_DisplayIfValidSession_b:=false;
      rtJMenu(JFC_DLITEM(p_DL.lpItem).lpPtr^).JMenu_ReadMore_b:=false;
...
const
  ...
  begin
    if saLeftStr(p_saLink,1)='?' then result:=garJVHost[p_Context.u2VHost].VHost_ServerURL+'/'+p_saLink else result:=p_saLink;
  end;
  ...
  //  iMethod:=0;
    ...
    Begin
      ...
      repeat
        ...
        u8FieldData:=rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_JSecPerm_ID;
        ...
        Else
        Begin
          bValidSessionOnly:=rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_ValidSessionOnly_b;
        End;

        bDisplayIfValidSession:=rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_DisplayIfValidSession_b;
        ...
        if bRenderAsLink and (not p_Context.bSessionValid) and  (not rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_DisplayIfNoAccess_b) then
        begin
          bRenderAsLink:=false;
        end;

        bRender:=bRenderAsLink;
        ...
        Begin
          ...
          Begin
            saCaption:=rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_Name;
          End
          Else
          Begin
          ...
          if bHere then
          begin
            p_saMenuTitle:='<script>document.title="'+rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_Name+'";</script>'+
              '<!--PAGETITLE "'+saCaption+'"-->';
              //'<!--MENUTITLE "'+saCaption+'"-->'+
            ...
            3: begin
              ...
              begin
                ...
                Begin
                  ...
                  if rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_NewWindow_b then
                  begin
                ...
                begin
                  ...
                  if rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_IconSmall_Theme_b then  saPanel+='THEME';
                ...
                begin
                  saPanel+='<img class="image" src="<? echo $JASDIRICON';
                  if rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_IconLarge_Theme_b then  saPanel+='THEME';
            ...
            4: begin
              ...
              begin
                ...
                Begin
                  saPanel+='</a>';
                End;
                if (length(rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_Data_en)<=128) or (false = rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_ReadMore_b) then
                begin
          ...
          cnRenderMethod_Theme: begin
            ...
            3: begin
              ...
              begin
                ...
                If bRenderAsLink Then
                Begin
                  saAnchor+='<a name="'+saSNRStr(rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_Name,' ','_')+'"></a>';
                  saAnchor+='<a href="'+saLinkScrub(saLink)+'"';
                  if rtJMenu(JFC_DLITEM(p_MENUDL.lpItem).lpPtr^).JMenu_NewWindow_b then
                  begin
                    saAnchor+=' target="_blank"';
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102446, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  saMenuTitle:='';
  MenuDL:=JFC_DL.CreateCopy(garJVHost[p_Context.u2VHost].MenuDL);
  ...
  Else
  Begin
    u8MenuID:=garJVHost[p_Context.u2VHost].VHost_DefaultTop_JMenu_ID;
  End;
  if p_Context.u8MenuTopID=0 then p_Context.u8MenuTopID:=garJVHost[p_Context.u2VHost].VHost_DefaultTop_JMenu_ID;
  ...
  {$IFDEF CACHEJASMENU }
  saPath:=saJASGetUserCacheDir(p_Context,p_Context.rJUser.JUser_JUser_UID);
  saCacheFileName:=p_Context.sLang+'_MT' + inttostr(p_Context.u8MenuTopID)+'_MID'+inttostr(u8MenuID)+'_USER'+inttostr(p_Context.rJUser.JUser_JUser_UID);
  ...
  {$ENDIF}
    ...
    bMenuRenderSuccessful:=bJMenuRender(
      ...
      garJVHost[p_Context.u2VHost].VHost_MenuRenderMethod,//3, // grJASConfig.u8DefaultThemeRenderMethod
  ...
  Begin
    ...
    p_Context.PAGESNRXDL.AppendItem_SNRPair('[@JASHASPANELS@]',TRIM(UPPERCASE(sTRueFalse(p_Context.bMenuHasPanels))));
    ...
    //rContext.PAGESNRXDL.AppendItem_SNRPair('<!--@JAS-  MENU@-->',saGetPage(rcontext, 'sys_page_htmlripper', 'menu_htab_3_0','',true));
  End
  Else
  Begin
