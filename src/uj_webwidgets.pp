...
Procedure WidgetBoolean(
  ...
  p_saSize: ansiString; // Widget Height Property - effect SELECT Tag's Size
  p_bEditMode: Boolean;
...
Procedure WidgetDateTime(
  ...
  p_saWidgetCaption: AnsiString;
  p_saDefaultValue: AnsiString;
  p_saWidgetDate: AnsiString; //< 'boolean' NOT ZERO '0' =MEANS TRUE
  p_saWidgetTime: AnsiString; //< 'boolean'
...
begin
  ...
  sa:=
    ...
    '  <img class="image" onclick="f'+p_saWidgetName+'FTA();" title="Click make this EXCLUDE" src="<? echo $JASDIRICONTHEME; ?>16/actions/edit-add.png" />'+
    '</div>'+
    '<div id="'+p_saWidgetName+'FTB" style="'+sDNot+'" />'+
    '  <img class="image" onclick="f'+p_saWidgetName+'FTB()" title="Click to make this INCLUDE" src="<? echo $JASDIRICONTHEME; ?>16/actions/edit-delete.png" />'+
    '</div>';
  result:=sa;
end;
//=============================================================================
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102661, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  Begin
    ...
    If p_saCols<>'' Then saResult+='cols="'+p_saCols+'" ';
    If p_saRows<>'' Then saresult+='rows="'+p_saRows+'" ';
  ...
  Begin
    ...
    If p_saCols<>'' Then saResult+='cols="'+p_saCols+'" ';
    If p_saRows<>'' Then saresult+='rows="'+p_saRows+'" >'+p_saDefaultValue+'</textarea>';
  end;
  If p_bEditMode Then
  begin
    saResult+='</textarea>';
    ...
    begin
      saResult+=
        ...
        '<img class="image" src="'+grJASConfig.saWebShareAlias+'img/icon/themes/'+saIconThemeToUse+'/'+'16/actions/encrypt.png" />'+
        '</a>&nbsp;&nbsp;'+
        '<a title="Decrypt" onclick="Decrypt_'+p_saWidgetName+'();">'+
        '<img class="image" src="'+grJASConfig.saWebShareAlias+'img/icon/themes/'+saIconThemeToUse+'/'+'16/status/dialog-password.png" />'+
        '</a>&nbsp;&nbsp;'+
        '<a title="Encode URI" onclick="EncodeURI_'+p_saWidgetName+'();">'+
        '<img class="image" src="'+grJASConfig.saWebShareAlias+'img/icon/themes/'+saIconThemeToUse+'/'+'16/actions/start.png" />'+
        '</a>&nbsp;&nbsp;'+
        '<a title="Decode URI" onclick="DecodeURI_'+p_saWidgetName+'();">'+
        '<img class="image" src="'+grJASConfig.saWebShareAlias+'img/icon/themes/'+saIconThemeToUse+'/'+'16/actions/stop.png" />'+
        '</a>&nbsp;&nbsp;'+
        '<a title="HTML render" onclick="render_'+p_saWidgetName+'();">'+
        '<img class="image" src="'+grJASConfig.saWebShareAlias+'img/icon/themes/'+saIconThemeToUse+'/'+'16/places/folder-remote.png" />'+
        '</a>&nbsp;&nbsp;'+
        '<a title="HTML render clear" onclick="renderclear_'+p_saWidgetName+'();">'+
        '<img class="image" src="'+grJASConfig.saWebShareAlias+'img/icon/themes/'+saIconThemeToUse+'/'+'16/places/folder.png" />'+
        '</a>';
    end;
  end;
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102664, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  begin
    ...
    begin
      saDefaultValue:=trim(UpCase(saTrueFalse(bVal(saDefaultValue))));
    end;
  end
  else
  begin
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102673, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  if p_saDefaultValue='00/00/00 12:00 AM' then p_saDefaultValue:='';


  bDate:=ival(p_saWidgetDate)<>0;
  bTime:=ival(p_saWidgetTime)<>0;
...
//           +'" value="'+saSNRStr(p_saDefaultValue,'"','&quot;') +'">';
  ...
  saResult+='<select size="'+p_saSize+'" name="'+p_saWidgetName+'" id="'+p_saWidgetName +'" ';
  if p_bMultiple then saResult+='multiple ';
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102700, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  Begin
    ...
    Begin
      ...
      begin
        sa+=saLeftStr(p_saDefaultValue,40)+'...';
      end
      else
      begin
        sa+=p_saDefaultValue;
