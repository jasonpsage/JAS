...
Function saJASGetUserCacheDir(p_Context: TCONTEXT; p_saJUser_JUser_UID: Ansistring):AnsiString;
//=============================================================================
{}
...
//{IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102646, sTHIS_ROUTINE_NAME);{ENDIF}
  ...
  safileName:=saZeroPadInt(u8Val(p_saJUser_JUser_UID),19)+'.txt';
  saFilePath:=garJVHostLight[p_Context.i4VHost].saCacheDir+'jas-menu'+csDOSSLASH;
  if not FileExists(saFilePAth) then CreateDir(saFilePath);
  If not bfileexistsintree(safileName,safileName,saFilePath, saRelPath,4,4) Then
  Begin
  ...
  End;
  Result:=saFilePath + csDOSSlash+saRelpath;
{$IFDEF DEBUGLOGBEGINEND}
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102649, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  Begin
    ...
    // The reason is that in the future, having a cookies based preference
    // feature is planned. I suspect this would have less priority 
    // than logged in user preferences (from database) so that for non system
    // users (anonymous web people) could have their own preferences stored
  ...
  Begin
    ...
    Begin
      JAS_Log(p_Context,cnLog_Error,200601252238,'Problem Seeking User Preference:'+
        saStr(p_u8PrefID)+' for user:'+ saStr(p_u8UserID),'Query: '+saQry,SOURCEFILE,DBC,rs);
  ...
  Begin
    ...
    saQry:='SELECT UsrPL_Value FROM juserpreflink '+
      'WHERE UsrPL_UserPref_ID='+DBC.saDBMSUIntScrub(p_u8PrefID)+' AND '+
    ...
    Begin
      JAS_Log(p_Context,cnLog_Error,200601252242,'Problem Seeking DEFAULT User Preference:'+
        saStr(p_u8PrefID),'Query: '+saQry,SOURCEFILE);
      //RenderHtmlErrorPage(p_Context);
    End;
  End; 
