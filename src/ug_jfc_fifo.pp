...
//=============================================================================
  UID: Integer;//< Unique Message Identifier within FIFO.
  ...
  {}
  iSrcEntity: integer;//< Source Entity (Type)
  iSrcID: integer;//< Source Entity ID
  ...
  {}
  iDestEntity: integer;//< Destination Entity (type)
  iDestID: integer;//< Destination Entity ID
...
//=============================================================================
  ...
  {}
  iAllocationSize: integer;//< How many messages for the Queue 
  // you wish to allocate at one time when the FIFO fills and 
  // more space is needed for an incoming message. 
  
  iInitialNoOfAllocationUnits: integer;//< How many allocation units to 
  ...
  iMaximumAllowedAllocationUnits: integer;//< If ZERO, this has no effect, however if bAddAllocationUnitsWhenFIFOFull
...
//=============================================================================
  ...
  pvt_iNextUID: integer;//< PRIVATE - Next Autonumber value to use 
  pvt_iHead: integer;//< PRIVATE - Tracks the start of where we are in the FIFO array
  pvt_iTail: integer;//< PRIVATE - Tracks the END of where we are in the FIFO
  pvt_iItems: integer;//< PRIVATE
  pvt_iAllocationSize: integer;//< PRIVATE
  pvt_iAllocatedUnits: Integer;//< PRIVATE
  pvt_iInitialNoOfAllocationUnits: integer;//< PRIVATE
  pvt_iMaximumAllowedAllocationUnits:integer;//< PRIVATE
  pvt_iSize: integer;//< PRIVATE
  ...
  pvt_iNumberTimesSendFailed: integer;//< PRIVATE
  pvt_iMaximumNumberOfItemsReached: integer;//< PRIVATE
  pvt_iMaximumNumberOfUnitsAllocated: integer;//< PRIVATE
  ...
  function iItems: Integer;//< Items in FIFO
  ...
  function iAllocationUnitsNotInUse: integer;
  ...
  function iReclaimUnusedAllocationUnits(p_iHowMany:Integer):integer;
  
  property NextUID:integer read pvt_iNextUID;//< Next Unique Identifier
  property HeadPos:integer read pvt_iHead;//< Head Position of the FIFO in the Array
  property TailPos:integer read pvt_iTail;//< Tail position of the FIFO in the Array 
  property Items:integer read pvt_iItems;//< Number of items in the Array
  property AllocationSize: integer read pvt_iAllocationSize;//<Number of items per Allocation Unit
  property AllocatedUnits: Integer read pvt_iAllocatedUnits;//<Number of units allocated 
  property InitialNoOfAllocationUnits: integer read pvt_iInitialNoOfAllocationUnits;//<Number of allocation units allocated when class instantiated. This serves as a minimum.
  property MaximumAllowedAllocationUnits: integer read pvt_iMaximumAllowedAllocationUnits;//<Maximum Allowed Allocation Units. 

  property SendFailures: integer read pvt_iNumberTimesSendFailed;//< This is the number of times messages didn't sucessfully get sent
  property MaximumNumberOfItemsReached: integer read pvt_iMaximumNumberOfItemsReached;//< Number of times the Fifo hit it's limit if limit set.
  property MaximumNumberOfUnitsAllocated: integer read pvt_iMaximumNumberOfUnitsAllocated;//< Maximum number of Units allocated through life of the FIFO

  property Size: integer read pvt_iSize;//< The Size of the FIFO.
...
begin
  ...
  if pvt_iAllocatedUnits<0 then pvt_iAllocatedUnits:=0;
...
//=============================================================================
function JFC_FIFO.iItems: Integer;
//=============================================================================
begin
  result:=pvt_iItems;
end;
...
begin
  ...
  p_Dest.iMsgType             :=          p_Src.iMsgType       ;
  p_Dest.iSrcID               :=          p_Src.iSrcID         ;
  p_Dest.lpSrcObj             :=          p_Src.lpSrcObj       ;
  //p_Dest.saSrcData            :=          p_Src.saSrcData      ;
  p_Dest.iDestID              :=          p_Src.iDestID        ;
...
begin
  ...
  p_rFIFOItem.iMsgType             :=          0;
  p_rFIFOItem.iSrcID               :=          0;
  p_rFIFOItem.lpSrcObj             :=          nil;
  //p_rFIFOItem.saSrcData            :=          '';
  p_rFIFOItem.iDestID              :=          0;
...
begin
  ...
  //begin
    ...
    begin
      //riteln('pp-fifo autonumber:',pvt_iNextUID,' tail:',pvt_iTail);
      pvt_aFIFOItem[pvt_iTail].UID:=pvt_iNextUID;
      pvt_iNextUID+=1;
    end;
...
// Use this function to see if it's be worth performing the 
// bReclaimUnusedAllocationUnits function if you like.
function JFC_FIFO.iAllocationUnitsNotInUse: integer;
//=============================================================================
begin
...
// depending on # of allocation units allocated - as it consolidates the 
// data)
function JFC_FIFO.iReclaimUnusedAllocationUnits(p_iHowMany:Integer):integer;
//=============================================================================
