...
Begin
  ...
  sa:=
    ' <TXLife xmlns="http://ACORD.org/Standards/Life/2">'+csCRLF+
    '   <UserAuthRequest>'+csCRLF+
    '     <UserLoginName>AdvantageOneAPIUser</UserLoginName>'+csCRLF+
    '     <UserPswd>'+csCRLF+
    '       <CryptType>MD5</CryptType>'+csCRLF+
    '       <CryptPswd>7AC37556FC714BA48594051A2095B626</CryptPswd>'+csCRLF+
    '     </UserPswd>'+csCRLF+
    '   </UserAuthRequest>'+csCRLF+
    ' </TXLife>'+csCRLF;
  ...
  try
    if HTTP.HTTPMethod('POST',saHost) then
    begin
      write(Http.headers.text);
      write(csINET_HDR_CONTENT_TYPE+csMIME_TextHtml +csCRLF);
      write('Content-length: ' + inttostr(length(sa)) + csCRLF);
      write(sa);
      writeln(csCRLF+csCRLF);
