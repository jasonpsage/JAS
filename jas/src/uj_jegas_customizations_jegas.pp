...
Unit uj_jegas_customizations_jegas;
//=============================================================================
...
{$DEFINE SOURCEFILE:='uj_jegas_customizations.pp'}
{$SMARTLINK ON}
{$PACKRECORDS 4}
{$MODE objfpc}
...
//*****************************************************************************
{}
// NOTE: bUserCustomations does not return success or failure: It is not an
// error flag. It just tells JAS that request was not off in left field,
// because if its way off for JAS and for the USEr Customizations, that
// might important from a security point of view. (They phishing/hacking?)
//
// bUserCustomization should follow this rule by the customizer:
// return TRUE ONLY if you handled the request, e.g. "It was one of yours!"
...
//=============================================================================
procedure userapi_archivedatabase(p_Context: TCONTEXT);
//=============================================================================
var 
  //xmlnode: JFC_XML;
  //xmlnode2: JFC_XML;
...
Begin
{$IFDEF ROUTINENAMES}sTHIS_ROUTINE_NAME:='userapi_archivedatabase(p_Context: TCONTEXT);';{$ENDIF}
{$IFDEF DEBUGLOGBEGINEND}
  DebugIn(sTHIS_ROUTINE_NAME,SourceFile);
{$ENDIF}
{$IFDEF DEBUGTHREADBEGINEND}p_Context.JThread.DBIN(201203102362,sTHIS_ROUTINE_NAME,SOURCEFILE);{$ENDIF}
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102363, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  with p_Context do begin
    ...
    // required for web application development: cook ies, form parameters,
    ...
    begin
      ...
      begin
        p_Context.u8ErrNo:=201012191703;
        saErrMsg:='Session Not Valid. Are you logged in?';
      end;
    end;
  
  end;
  p_Context.CGIENV.uHTTPResponse:=200;

{$IFDEF DEBUGLOGBEGINEND}
  DebugOut(sTHIS_ROUTINE_NAME,SourceFile);
{$ENDIF}
{$IFDEF DEBUGTHREADBEGINEND}p_Context.JThread.DBOUT(201203102364,sTHIS_ROUTINE_NAME,SOURCEFILE);{$ENDIF}
end;
//=============================================================================
...
{$IFDEF TRACKTHREAD}p_Context.JThread.TrackThread(201203102369, sTHIS_ROUTINE_NAME);{$ENDIF}
  ...
  with p_Context do begin
    ...
    p_Context.CGIENV.uHTTPResponse:=200;// Not setting this will cause an error.
  end;
{$IFDEF DEBUGLOGBEGINEND}
  DebugOut(sTHIS_ROUTINE_NAME,SourceFile);
{$ENDIF}
...
donehere: ;
  result:=false;//for now - there is nothing - so we handled nothing
  // (the request wasn't for us - we have nothing here) return false because we
  // didnt handle the request
{$IFDEF DEBUGLOGBEGINEND}
  DebugOut(sTHIS_ROUTINE_NAME,SourceFile);
{$ENDIF}
